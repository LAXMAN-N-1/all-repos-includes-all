...
class _RentalTrackingScreenState extends State<RentalTrackingScreen> {
  ...
  void initState() {
    super.initState();
    _rentalExpiry = widget.rentalExpiry ?? DateTime.now().add(const Duration(hours: 5));
  }
  ...
  void _checkAndShowNotification(RentalTimer timer) {
    ...
    if (hours == 24 && !_notifiedMarkers.contains(24)) {
      _triggerOverlay(
          "24H EXPIRY WARNING", "Nearby: Downtown Energy Hub (0.8km)");
      _notifiedMarkers.add(24);
    } else if (hours == 12 && !_notifiedMarkers.contains(12)) {
      _triggerOverlay("12H EXPIRY WARNING", "Nearby: Greenway Point (1.4km)");
      _notifiedMarkers.add(12);
    } else if (hours <= 1 &&
        hours >= 0 &&
        mins <= 60 &&
        !_notifiedMarkers.contains(1)) {
      _triggerOverlay("FINAL HOUR WARNING", "Immediate swap at Metro Station!");
          ...
          children: [
            ...
            Expanded(
              child: _buildDiagnosticItem(
                ...
                'Expires: ${DateFormat('dd MMM, hh:mm a').format(_rentalExpiry)}',
  ...
  Widget _buildRecommendedStations() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('Recommended Swap Stations',
            style: TextStyle(
                color: AppTheme.textSecondary,
                fontSize: 11,
                fontWeight: FontWeight.bold)),
        const SizedBox(height: 12),
        _buildStationMiniItem(
            'Downtown Energy Hub', '0.8 km', '12/50', 'Open 24/7'),
        const SizedBox(height: 8),
        _buildStationMiniItem(
            'Greenway Point', '1.4 km', '5/20', 'Until 10 PM'),
      ],
    );
  }

  Widget _buildStationMiniItem(
      String name, String dist, String avail, String hours) {
    return Container(
      padding: const EdgeInsets.all(20),
      decoration: BoxDecoration(
