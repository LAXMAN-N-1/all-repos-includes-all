...
final dealerQuickStatsProvider = FutureProvider<DealerStatsDto>((ref) async {
  final dio = ref.watch(dioProvider);
  try {
    final res = await dio.get(ApiConstants.dealerStats);
    final data = res.data as Map<String, dynamic>;
    return DealerStatsDto(
      availableBatteries: data['available_batteries'] ?? 0,
      totalBatteries: data['total_batteries'] ?? 0,
      ongoingRentals: data['ongoing_rentals'] ?? 0,
      currentSwaps: data['current_swaps'] ?? 0,
      avgRating: (data['avg_rating'] ?? 0.0).toDouble(),
      stationCount: data['station_count'] ?? 0,
    );
  } catch (e) {
    log('DealerStats error: $e');
    rethrow;
  }
});
...
final dealerBatteriesProvider =
  ...
  try {
    ...
    final res =
        await dio.get(ApiConstants.dealerBatteries, queryParameters: params);
    ...
    return rawList
        .map((e) => BatteryDto(
              ...
              status: e['status']?.toString() ?? 'available',
              chargePercentage: (e['current_charge'] ?? 100.0).toDouble(),
              healthPercentage: (e['health_percentage'] ?? 100.0).toDouble(),
              cycleCount: e['cycle_count'] ?? 0,
              ...
              daysIdle: e['days_idle'] ?? 0,
              faultDescription: e['fault_description']?.toString(),
...
class SwapStateNotifier extends StateNotifier<SwapScreenState> {
  ...
  Future<void> refresh() async {
    ...
    try {
      ...
      final stationsRaw = (stationRes.data as List? ?? [])
      ...
      final swapsRes =
          await dio.get(ApiConstants.dealerSwapsList, queryParameters: params);
      final swapsRaw = (swapsRes.data as List? ?? [])
          .whereType<Map<String, dynamic>>()
          .toList();

      final rentalsRes = await dio.get(ApiConstants.dealerActiveRentals,
          queryParameters: params);
      final rentalsRaw = (rentalsRes.data as List? ?? [])
          .whereType<Map<String, dynamic>>()
          .toList();

      final batteriesRes =
          await dio.get(ApiConstants.dealerBatteries, queryParameters: params);
      final batteriesRaw = (batteriesRes.data as List? ?? [])
          .whereType<Map<String, dynamic>>()
          .toList();
