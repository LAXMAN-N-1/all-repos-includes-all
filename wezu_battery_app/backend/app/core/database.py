...
def ensure_runtime_tables_compatibility() -> None:
    ...
    required_tables = {
        ...
        "payment_methods": PaymentMethod.__table__,
        "biometric_credentials": BiometricCredential.__table__,
    }
...
def _safe_session_rollback(session: Session, *, context: str) -> None:
    try:
        session.rollback()
    except SQLAlchemyError as exc:
        logger.warning("db.session_rollback_failed", context=context, error=str(exc))
        try:
...
def _safe_session_close(session: Session, *, context: str) -> None:
    try:
        session.close()
    except SQLAlchemyError as exc:
        logger.warning("db.session_close_failed", context=context, error=str(exc))
        try:
            bind = session.get_bind()
            if bind is not None:
                bind.dispose()
...
def get_db():
    ...
    try:
        ...
        _safe_session_rollback(session, context="request_exception")
        raise
    finally:
        _safe_session_close(session, context="request_finalizer")
