...
class BiometricLoginRequest(BaseModel):
    ...
    @model_validator(mode="after")
    def validate_login_shape(self) -> Self:
        has_device_flow = bool((self.device_id or "").strip() and (self.biometric_token or "").strip())
        has_credential_flow = bool(
            (self.credential_id or "").strip()
            and (self.signature or "").strip()
            and (self.challenge or "").strip()
        )
        if has_device_flow or has_credential_flow:
            return self
        raise ValueError(
            "Provide either device_id + biometric_token or credential_id + signature + challenge"
        )
