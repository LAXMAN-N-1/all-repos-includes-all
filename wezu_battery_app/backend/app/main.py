...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        # ── DB warmup ping + legacy normalization ─────────────────────────
        # Runs once on startup so the first customer login doesn't pay the
        # 5-20s NeonDB cold-start + table-scan penalty.
        if not startup_without_db:
            try:
                from app.core.database import engine as _engine
                from sqlmodel import Session as _Session, text as _text
                from app.api.v1.customer_auth import _normalize_legacy_user_type_values
                with _Session(_engine) as _db:
                    _db.execute(_text("SELECT 1"))   # warm up the connection pool
                    _normalize_legacy_user_type_values(_db)  # run once at startup
                logger.info("startup.db_warmup_done")
            except Exception as _exc:
                logger.warning("startup.db_warmup_failed", error=str(_exc))
