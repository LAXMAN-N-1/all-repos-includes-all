...
_legacy_normalization_done = False  # Run at most once per process
...
def _normalize_legacy_user_type_values(db: Session) -> None:
    ...
    Runs only ONCE per process lifetime to avoid a full-table scan on every login.
    """
    global _legacy_normalization_done
    if _legacy_normalization_done:
        return
    _legacy_normalization_done = True
    ...
    try:
        ...
        db.commit()
        logger.warning(
            "Normalized legacy lowercase user_type values",
            extra={"updated_rows": len(legacy_rows)},
        )
    except Exception as e:
        logger.warning(f"Legacy normalization skipped: {e}")
