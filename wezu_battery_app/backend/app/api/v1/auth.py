...
async def biometric_login(
    ...
    credential = None
    using_device_token_flow = bool(
        (data.device_id or "").strip() and (data.biometric_token or "").strip()
    )

    if using_device_token_flow:
        credential = db.exec(
            select(BiometricCredential).where(
                BiometricCredential.device_id == data.device_id,
                BiometricCredential.public_key == data.biometric_token,
            )
        ).first()
    else:
        credential = db.exec(
            select(BiometricCredential).where(
                BiometricCredential.credential_id == data.credential_id
            )
        ).first()
    ...
    is_valid = False
    if using_device_token_flow:
        # Mobile fallback contract: device_id + biometric_token.
        # We trust OS biometric gating on-device and verify token/device match.
        is_valid = True
    else:
        is_valid = AuthService.verify_biometric_signature(
            db,
            credential.user_id,
