...
from app.models.access_assignment import StationStaffAssignment
...
DEALER_ALL_STATION_ROLES = {"dealer_owner", "dealer_manager"}
DEALER_STATION_SCOPED_ROLES = {
...
def can_access_all_dealer_stations(user: User, dealer: DealerProfile) -> bool:
    if getattr(user, "is_superuser", False):
        return True
    if dealer.user_id == user.id:
        return True
    role_names = _user_role_names(user)
    return bool(role_names & DEALER_ALL_STATION_ROLES)


def accessible_station_ids_for_user(
    db: Session,
    user: User,
    dealer: DealerProfile,
) -> list[int]:
    all_station_ids = dealer_station_ids(db, dealer)
    if can_access_all_dealer_stations(user, dealer):
        return all_station_ids

    rows = db.exec(
        select(StationStaffAssignment.station_id).where(
            StationStaffAssignment.dealer_id == dealer.id,
            StationStaffAssignment.user_id == user.id,
            StationStaffAssignment.is_active == True,  # noqa: E712
        )
    ).all()
    allowed = {int(row) for row in rows}
    return [station_id for station_id in all_station_ids if station_id in allowed]


def assert_station_in_user_scope(
    db: Session,
    *,
    user: User,
    dealer: DealerProfile,
    station_id: int,
    request: Optional[Request] = None,
    endpoint: str = "dealer_station",
) -> Station:
    station = db.get(Station, station_id)
    allowed_station_ids = set(accessible_station_ids_for_user(db, user, dealer))
    if not station or station.dealer_id != dealer.id or station_id not in allowed_station_ids:
        log_scope_violation(
            actor_id=user.id,
            dealer_id=dealer.id,
            target_id=station_id,
            endpoint=endpoint,
            reason="station_not_in_dealer_scope",
            request=request,
        )
