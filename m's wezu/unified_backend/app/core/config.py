...
class Settings(BaseSettings):
    ...
    SQL_SLOW_QUERY_IGNORE_PATTERNS: Union[list[str], str] = ["from order_realtime_outbox"]
    ...
    ANALYTICS_REPORT_TTL_SECONDS: int = 14400  # 4 hours; controls Redis TTL for queued dashboard reports
    ...
    AUTH_PROVIDER: Literal["local", "supabase", "hybrid"] = "supabase"
    SUPABASE_URL: Optional[str] = None
    SUPABASE_JWKS_URL: Optional[str] = None
    SUPABASE_JWT_ISSUER: Optional[str] = None
    SUPABASE_JWT_AUDIENCE: Optional[str] = "authenticated"
    SUPABASE_ALLOWED_ALGORITHMS: Union[list[str], str] = ["ES256", "RS256"]
    ...
    AUTH_REQUIRE_IAT_CLAIM: bool = True
    AUTH_IAT_FUTURE_SKEW_SECONDS: int = 120
    WEBSOCKET_ALLOW_QUERY_TOKEN: bool = False
    KYC_HMAC_SECRET: Optional[str] = None
    ...
    GST_RATE: float = 0.18  # Goods and Services Tax rate (India default: 18%)
    ...
    ORDER_WEBSOCKET_REALTIME_ENABLED: bool = False
    ...
    SCHEDULER_LOCK_FILE: Optional[str] = None
    ...
    LOG_EXCLUDE_PATHS: Union[list[str], str] = ["/health", "/ready", "/live"]
    ...
    DB_NOOP_MUTATION_IGNORE_TABLES: Union[list[str], str] = ["alembic_version"]
    ...
    ALLOWED_HOSTS: Union[list[str], str] = ["localhost", "127.0.0.1", "[::1]", "testserver"]
    ENABLE_TRUSTED_HOST_MIDDLEWARE: bool = False
    TRUST_X_FORWARDED_HOST: bool = True
    FORWARDED_ALLOW_IPS: Union[list[str], str] = ["127.0.0.1/32", "::1/128", "172.16.0.0/12"]
    TRUSTED_PROXY_CIDRS: Union[list[str], str] = [
    ...
    APP_WORKER_COUNT: int = 2
    POSTGRES_MAX_CONNECTIONS: Optional[int] = None
    ALLOW_TEST_OTP_BYPASS: bool = False
    LOGISTICS_SCHEMA_CHECK_ENABLED: bool = True
    LOGISTICS_SCHEMA_AUTOPATCH_ENABLED: bool = True
    LOGISTICS_SCHEMA_STRICT: bool = False
    SCHEMA_DRIFT_MISSING_TABLES_AS_ERROR: bool = False
    SCHEMA_DRIFT_IGNORED_TABLES: Union[list[str], str] = []
    SOFT_DELETE_GLOBAL_FILTER_ENABLED: bool = True
    ...
    PASSKEY_ORIGINS: Union[list[str], str] = []
    PASSKEY_ANDROID_RELATIONS: Union[list[str], str] = [
        "delegate_permission/common.handle_all_urls",
        "delegate_permission/common.get_login_creds",
    ]
    PASSKEY_ANDROID_PACKAGE_NAME: str = "com.example.frontend_logistic"
    PASSKEY_ANDROID_SHA256_CERT_FINGERPRINTS: Union[list[str], str] = [
    ...
    def _normalize_database_url(cls, value: Any) -> str:
        ...
        # Preserve sqlite URL shape exactly (e.g. sqlite:///./app.db).
    ...
    def _validate_supabase_auth(self) -> "Settings":
        if self.AUTH_PROVIDER not in {"supabase", "hybrid"}:
            return self
        # In Supabase modes we require endpoint configuration in all
        # non-test environments so the app fails fast on misconfiguration.
        enforce_urls = (self.ENVIRONMENT or "").lower() != "test"
        if enforce_urls and not self.SUPABASE_URL and not self.SUPABASE_JWKS_URL:
            raise ValueError(
        ...
        if not (self.SUPABASE_JWT_AUDIENCE or "").strip():
            raise ValueError("SUPABASE_JWT_AUDIENCE must be configured")
        if self.AUTH_IAT_FUTURE_SKEW_SECONDS < 0:
            raise ValueError("AUTH_IAT_FUTURE_SKEW_SECONDS must be >= 0")
        return self
    ...
    def _validate_production_secrets(self) -> "Settings":
        ...
        is_production = (self.ENVIRONMENT or "").strip().lower() == "production"
        if is_production:
            if self.AUTH_PROVIDER in {"supabase", "hybrid"} and not (
                self.SUPABASE_SERVICE_ROLE_KEY or ""
            ).strip():
                raise ValueError(
                    "SUPABASE_SERVICE_ROLE_KEY must be set in production for "
                    "Supabase Admin Auth operations"
                )
            if not (self.RATE_LIMIT_STORAGE_URL or "").strip():
                if getattr(self, "REDIS_URL", None):
                    self.RATE_LIMIT_STORAGE_URL = self.REDIS_URL
                else:
                    raise ValueError("RATE_LIMIT_STORAGE_URL must be set in production")
            if not (self.KYC_HMAC_SECRET or "").strip():
                if getattr(self, "SECRET_KEY", None):
                    self.KYC_HMAC_SECRET = self.SECRET_KEY
                else:
                    raise ValueError("KYC_HMAC_SECRET must be set in production")
            if (self.SCHEDULER_LOCK_FILE or "").strip():
                raise ValueError("SCHEDULER_LOCK_FILE is deprecated; use Redis distributed lock settings")
            if self.WEBSOCKET_ALLOW_QUERY_TOKEN:
                raise ValueError(
                    "WEBSOCKET_ALLOW_QUERY_TOKEN must be false in production; "
                    "use Authorization or Sec-WebSocket-Protocol bearer token transport"
                )

        worker_count = int(
            (os.getenv("WEB_CONCURRENCY") or os.getenv("GUNICORN_WORKERS") or self.APP_WORKER_COUNT or 2)
        )
        pg_max_connections = self.POSTGRES_MAX_CONNECTIONS
        if pg_max_connections and pg_max_connections > 0:
            projected_pool = (self.DB_POOL_SIZE + self.DB_MAX_OVERFLOW) * max(1, worker_count)
            if projected_pool > int(pg_max_connections * 0.8):
                raise ValueError(
                    "DB pool sizing exceeds 80% of Postgres max_connections "
                    f"(projected={projected_pool}, max={pg_max_connections})"
                )
        return self
