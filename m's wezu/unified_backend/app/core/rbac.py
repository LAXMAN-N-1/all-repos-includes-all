...
# Explicit mapping from UserType enum values to canonical role names.
# This is the single source of truth — keep in sync with UserType in models/user.py.
# Every UserType value MUST appear here so that adding a new UserType produces an
# immediate KeyError rather than silently resolving to a role name that matches nothing.
USERTYPE_TO_CANONICAL_ROLE: dict[str, str] = {
    "customer": "customer",
    "admin": "operations_admin",
    "dealer": "dealer_owner",
    "dealer_staff": "dealer_manager",
...
class RoleWindow:
    ...
    def is_active(self, now: datetime | None = None) -> bool:
        at = ensure_utc_aware(now or utcnow())
        effective_from = ensure_utc_aware(self.effective_from)
        expires_at = ensure_utc_aware(self.expires_at)

        if effective_from and effective_from > at:
            return False
