...
from app.core.rbac import USERTYPE_TO_CANONICAL_ROLE


BACKUP_SUFFIX = datetime.now(UTC).strftime("%Y%m%d_%H%M%S")
ALLOWED_BACKUP_TABLES = {"roles", "permissions", "role_permissions", "user_roles", "admin_user_roles"}
...
def _backup_tables(session: Session) -> None:
    for table_name in ("roles", "permissions", "role_permissions", "user_roles", "admin_user_roles"):
        ...
        if table_name not in ALLOWED_BACKUP_TABLES:
            raise ValueError(f"Disallowed table for backup: {table_name}")
        backup_name = f"{table_name}_backup_{BACKUP_SUFFIX}"
        if not _IDENTIFIER_RE.match(backup_name):
            raise ValueError(f"Unsafe backup table name: {backup_name}")
...
def _default_role_name_for_user(user_type: str | None, is_superuser: bool) -> str:
    if is_superuser:
        return "super_admin"
    key = (user_type or "customer").lower()
    return USERTYPE_TO_CANONICAL_ROLE.get(key, "customer")
