...
def _extract_metadata_assignment(node: ast.stmt) -> tuple[str, object] | None:
    """Return (name, value) for top-level revision metadata assignments."""
    ...
    if isinstance(node, ast.AnnAssign) and isinstance(node.target, ast.Name):
        # Alembic commonly emits annotated assignments:
        #   revision: str = "abcd"
        #   down_revision: Union[str, None] = ("a", "b")
        value = _extract_literal(node.value) if node.value is not None else None
...
def _parse_revision_file(path: Path) -> _RevisionFile | None:
    ...
    for node in tree.body:
        metadata = _extract_metadata_assignment(node)
        if metadata is None:
            continue
        key, value = metadata
        if key == "revision":
