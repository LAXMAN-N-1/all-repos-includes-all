...
class AuthService:
    ...
    async def verify_apple_token(token: str):
        try:
            ...
            logger.warning("auth.apple_token_invalid", exc_info=True)
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
    ...
    async def verify_facebook_token(token: str):
        try:
            ...
            logger.warning("auth.facebook_token_invalid", exc_info=True)
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
                detail="invalid_token",
            )
    ...
    def verify_supabase_access_token(cls, token: str) -> dict[str, Any]:
        ...
        if not audience:
            raise SupabaseTokenValidationError("token_invalid", "Supabase audience is not configured")
    ...
    def update_user_session(
        ...
        if session:
            # Update existing session — preserve old JTI in previous_token_id
            # so validate_session can accept an idempotent retry if the client
            # never received the rotated tokens due to a network failure.
            session.previous_token_id = session.token_id  # grace-window fallback
            session.previous_token_issued_at = datetime.now(UTC)
    ...
    def validate_session(db: Session, token: str, is_refresh: bool = False):
        ...
        # Primary lookup: current active token JTI / access sid.
        is_grace_window_retry = False
        session = db.exec(
            select(UserSession)
        ...
        # Grace-window fallback: the backend may have rotated the session but
        # the client never received the new tokens (network failure).  Accept
        # the old JTI for 90 s via previous_token_id so the client can retry.
        if not session and is_refresh:
            session = db.exec(
                select(UserSession)
                .where(UserSession.previous_token_id == token_id)
                .where(UserSession.is_active == True)
            ).first()
            if session:
                previous_issued = ensure_utc_aware(session.previous_token_issued_at)
                if previous_issued is None or now_utc - previous_issued > timedelta(seconds=90):
                    return None
                is_grace_window_retry = True
        ...
        if session_expires_at and session_expires_at < now_utc:
        ...
        # Hash check — skip for grace-window retries because the stored hash
        # is already the *new* token's hash; the client still holds the old one.
        if is_refresh and session.refresh_token_hash and not is_grace_window_retry:
    ...
    def revoke_session(db: Session, token: str) -> bool:
        ...
        session = None
        if token_type == "access" and sid:
            try:
                session = db.get(UserSession, int(sid))
            except (TypeError, ValueError):
                session = db.exec(
                    select(UserSession).where(UserSession.token_id == str(sid))
                ).first()
        elif token_type == "refresh" and refresh_jti:
            session = db.exec(
                select(UserSession).where(UserSession.token_id == str(refresh_jti))
            ).first()
        elif sid:
    ...
    @staticmethod
    def rotate_refresh_session_tokens(
        db: Session,
        refresh_token: str,
        request: Request | None = None,
    ) -> tuple[str, str, Any]:
        """
        Canonical refresh pipeline:
        - validate refresh session + hash
        - rotate refresh token and session binding
        - issue new access token bound to stable session.id sid
        """
        ...
        try:
            session = AuthService.validate_session(db, refresh_token, is_refresh=True)
            if not session:
                raise HTTPException(
                    status_code=status.HTTP_401_UNAUTHORIZED,
            ...
            payload = jwt.decode(refresh_token, settings.SECRET_KEY, algorithms=[settings.ALGORITHM])
            if str(payload.get("type") or "").lower() != "refresh":
                raise HTTPException(
                    status_code=status.HTTP_401_UNAUTHORIZED,
                    detail="token_invalid",
                    headers=auth_headers,
    ...
    @staticmethod
    def initiate_2fa_setup(db: Session, user) -> dict:
    ...
    @staticmethod
    def verify_and_enable_2fa(db: Session, user, code: str) -> list[str] | None:
        """Verify TOTP against stored secret, enable 2FA, return backup codes."""
        from datetime import datetime, UTC
        from app.models.two_factor_auth import TwoFactorAuth
        try:
            import pyotp
    ...
    @staticmethod
    def is_two_factor_enabled(db: Session, user_id: int) -> bool:
        from app.models.two_factor_auth import TwoFactorAuth

        record = db.exec(
            select(TwoFactorAuth).where(
                TwoFactorAuth.user_id == user_id,
                TwoFactorAuth.is_enabled == True,  # noqa: E712
            )
        ).first()
        return bool(record)
    ...
    @staticmethod
    def disable_two_factor(db: Session, user) -> None:
        from datetime import datetime, UTC
        from app.models.two_factor_auth import TwoFactorAuth
