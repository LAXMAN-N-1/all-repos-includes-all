...
class EmailService:
    ...
    def send_email(to_email: str, subject: str, content: str) -> bool:
        if not to_email or "@" not in to_email:
            logger.warning("Email not sent: invalid recipient '%s'", _mask_target(to_email))
            return False
        ...
        if not emails_enabled or not has_sendgrid_key:
            logger.info("MOCK EMAIL to %s: %s", _mask_target(to_email), subject)
        ...
        try:
            ...
            logger.exception("Error sending email to %s", _mask_target(to_email))
            return False
