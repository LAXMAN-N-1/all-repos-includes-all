...
from app.db.model_schema_drift import get_model_schema_drift_report
...
class StartupDiagnosticsService:
    ...
    def _build_report() -> dict[str, Any]:
        components = {
            ...
            "schema_drift": StartupDiagnosticsService._schema_drift_component(),
    ...
    @staticmethod
    def _schema_drift_component() -> dict[str, Any]:
        required = not bool(getattr(settings, "ALLOW_START_WITHOUT_DB", False))
        report = get_model_schema_drift_report(emit_logs=False)

        if report.check_error:
            return {
                "status": "check_failed",
                "required": required,
                "details": f"Schema drift check failed: {report.check_error}",
            }

        missing_tables = list(report.actionable_missing_tables)
        if missing_tables:
            return {
                "status": "drifted",
                "required": required,
                "details": {
                    ...
                    "remedy": "Run alembic upgrade head (revision d4f8c1a9b2e3 or newer).",
                },
            }
        ...
        return {
            ...
            "details": {
                ...
                "missing_tables": [],
                "ignored_missing_table_count": len(report.ignored_missing_tables),
            },
        }
        ...
        if ttl_seconds > 0:
            with StartupDiagnosticsService._cache_lock:
                if (
                    ...
                    age_seconds = (
                        datetime.now(UTC) - StartupDiagnosticsService._cached_generated_at
                    ).total_seconds()
            ...
            try:
                ...
                with StartupDiagnosticsService._cache_lock:
                    ...
                    StartupDiagnosticsService._cached_generated_at = datetime.now(UTC)
                return report
            finally:
    ...
    def enforce_required_dependencies() -> None:
        ...
        if failures:
            ...
            failure_details = {
                name: (components.get(name) or {}).get("details")
