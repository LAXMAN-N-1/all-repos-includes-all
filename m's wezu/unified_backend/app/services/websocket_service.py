...
def _tenant_order_key(
    ...
    *,
    ...
    if global_scope:
        return f"global:{order_id}"
    if tenant_id is None:
        raise ValueError("tenant_id is required for tenant-scoped subscriptions")
    return f"tenant:{int(tenant_id)}:{order_id}"


def _tenant_inventory_key(
    location_type: str,
    location_id: int,
    *,
    tenant_id: Optional[int] = None,
    ...
    base = _inventory_location_key(location_type, location_id)
    if global_scope:
        return f"global:{base}"
    if tenant_id is None:
        raise ValueError("tenant_id is required for tenant-scoped subscriptions")
    return f"tenant:{int(tenant_id)}:{base}"
...
class ConnectionManager:
    ...
    async def subscribe_all_orders(
        self,
        user_id: int,
        *,
        global_scope: bool = False,
        ...
        key = _tenant_order_key(
            "*",
            tenant_id=tenant_id,
            global_scope=global_scope,
        )
        if key not in self.order_subscriptions:
            self.order_subscriptions[key] = set()
        self.order_subscriptions[key].add(user_id)
        logger.info("User %s subscribed to all order updates (%s)", user_id, key)
    ...
    async def subscribe_order(
        ...
        *,
        ...
        if not order_key:
            return
        scoped_key = _tenant_order_key(
            order_key,
            tenant_id=tenant_id,
            global_scope=global_scope,
        )
        if scoped_key not in self.order_subscriptions:
            self.order_subscriptions[scoped_key] = set()
        self.order_subscriptions[scoped_key].add(user_id)
        logger.info("User %s subscribed to order %s updates (%s)", user_id, order_key, scoped_key)

    async def unsubscribe_order(
        self,
        user_id: int,
        order_id: str,
        *,
        ...
        subscribers = self.order_subscriptions.get(scoped_key)
        if not subscribers:
            return
        subscribers.discard(user_id)
        if not subscribers:
            del self.order_subscriptions[scoped_key]
        logger.info("User %s unsubscribed from order %s updates (%s)", user_id, order_key, scoped_key)
    ...
    async def broadcast_order_update(self, event: Dict[str, Any]):
        ...
        # Global-admin cross-tenant subscribers are explicit.
        target_users.update(self.order_subscriptions.get("global:*", set()))
        if order_id:
            target_users.update(self.order_subscriptions.get(f"global:{order_id}", set()))
        ...
        # Tenant-scoped delivery requires tenant_id on the event payload.
        if tenant_id is not None:
            target_users.update(self.order_subscriptions.get(f"tenant:{tenant_id}:*", set()))
            if order_id:
                target_users.update(self.order_subscriptions.get(f"tenant:{tenant_id}:{order_id}", set()))
    ...
    async def broadcast_inventory_update(self, event: Dict[str, Any]):
        ...
        for location in raw_locations:
            ...
            try:
                ...
                continue
            target_users.update(
                self.inventory_subscriptions.get(
                    _tenant_inventory_key(normalized_type, location_id, global_scope=True),
                    set(),
                )
            )
            if tenant_id is not None:
