...
def _render_pdf_bytes(payload: dict[str, Any]) -> bytes:
    ...
    story: list[Any] = []
    generated_at_utc = datetime.now(UTC).isoformat(timespec="seconds") + "Z"
...
class AnalyticsReportService:
    ...
    def create_download_token(report_id: str, *, expires_at: Optional[datetime] = None) -> str:
        ...
        if token_expiry is None or token_expiry <= datetime.now(UTC):
    ...
    def run_dashboard_report_job(
        ...
        try:
            ...
            detail_message = f"{exc.__class__.__name__}: {exc}" if str(exc) else repr(exc)
            logger.exception(
                "analytics.report_job.failed",
                report_id=report_id,
            ...
            try:
                ...
                logger.exception(
                    "analytics.report_job.failure_persist_failed",
                    report_id=report_id,
                )
