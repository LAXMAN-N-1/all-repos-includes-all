...
class RequestAuditQueueService:
    ...
    def enqueue(
        ...
        *,
        ...
        try:
            ...
            self._dropped += 1
            persisted = self._persist_overflow_event(event)
            if self._dropped % self._drop_warn_every == 0:
                logger.warning(
                    "Request audit queue full; dropped=%s persisted_overflow=%s enqueued=%s maxsize=%s",
                    self._dropped,
                    self._overflow_persisted,
                    self._enqueued,
                    self._queue.maxsize,
                )
            return persisted
    ...
    def stats(self) -> Dict[str, Any]:
        return {
            ...
            "overflow_persisted": self._overflow_persisted,
            "flushed": self._flushed,
            "flush_errors": self._flush_errors,
        }

    def _persist_overflow_event(self, event: Dict[str, Any]) -> bool:
        payload = dict(event)
        metadata = dict(payload.get("meta_data") or {})
        metadata["queue_overflow_fallback"] = True
        payload["meta_data"] = metadata
        try:
            with Session(engine) as db:
                db.add(AuditLog(**payload))
                db.commit()
            self._overflow_persisted += 1
            return True
        except Exception as exc:
            logger.error("Failed audit overflow fallback write: %s", exc)
            return False

    def _maybe_warn_queue_depth(self) -> None:
        if self._queue.maxsize <= 0:
            return
        depth_ratio = self._queue.qsize() / float(self._queue.maxsize)
        if depth_ratio >= 0.8 and not self._depth_warned:
            logger.warning(
                "Request audit queue depth above 80%% (size=%s max=%s)",
                self._queue.qsize(),
                self._queue.maxsize,
            )
            self._depth_warned = True
        elif depth_ratio < 0.5 and self._depth_warned:
            self._depth_warned = False
