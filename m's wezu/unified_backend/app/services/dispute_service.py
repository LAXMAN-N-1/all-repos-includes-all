...
class DisputeService:
    ...
    @staticmethod
    def create_dispute(
        db: Session, settlement_id: int, requester_user_id: int, reason: str
    ) -> SettlementDispute:
        """Open a dispute on a settlement."""
        settlement = db.get(Settlement, settlement_id)
        if not settlement:
            raise ValueError("Settlement not found")
        expected_dealer_profile_id, _ = SettlementService.resolve_dealer_scope_ids(
            db, requester_user_id
        )
        if settlement.dealer_id != expected_dealer_profile_id:
