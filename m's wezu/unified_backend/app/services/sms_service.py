...
class SMSService:
    ...
    def send_sms(phone: str, message: str) -> bool:
        ...
        if not normalized_phone:
            logger.warning("SMS not sent: invalid phone number '%s'", _mask_target(phone))
            return False
        ...
        if (
            ...
            try:
                ...
                logger.exception("Twilio SMS send failed for %s", _mask_target(normalized_phone))
                if SMSService._is_non_production():
                    logger.info("Using mock SMS success fallback in non-production")
                    return True
                return False

        logger.info("MOCK SMS to %s: %s", _mask_target(normalized_phone), cleaned_message)
