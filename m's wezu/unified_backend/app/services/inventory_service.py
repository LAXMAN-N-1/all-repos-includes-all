...
class InventoryService:
    ...
    def confirm_receipt(
        ...
        if tenant_id is not None and int(getattr(transfer, "tenant_id", 0) or 0) != int(tenant_id):
            raise ValueError("Transfer not found")
        
        normalized_status = str(transfer.status).strip().lower().replace("-", "_")
        if normalized_status in {"received", "completed"}:
            return transfer
