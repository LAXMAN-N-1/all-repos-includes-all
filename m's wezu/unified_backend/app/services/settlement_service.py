...
class SettlementService:
    @staticmethod
    def resolve_dealer_scope_ids(db: Session, dealer_actor_id: int) -> tuple[int, int]:
        """
        Resolve canonical dealer scope identifiers.

        Returns:
            (dealer_profile_id, dealer_owner_user_id)

        Notes:
            - Canonical settlement ownership is dealer_profiles.id.
            - Commission/chargeback ownership remains users.id.
            - Fallback keeps compatibility for legacy/test data where no
              DealerProfile exists yet.
        """
        profile = db.get(DealerProfile, dealer_actor_id)
        if profile and profile.id is not None:
            return int(profile.id), int(profile.user_id)

        profile = db.exec(
            select(DealerProfile).where(DealerProfile.user_id == dealer_actor_id)
        ).first()
        if profile and profile.id is not None:
            return int(profile.id), int(profile.user_id)

        return int(dealer_actor_id), int(dealer_actor_id)
    ...
    def generate_monthly_settlement(
        ...
        dealer_profile_id, dealer_owner_user_id = SettlementService.resolve_dealer_scope_ids(
            db, dealer_id
        )
        ...
        existing = db.exec(
            select(Settlement).where(
                Settlement.dealer_id == dealer_profile_id,
        ...
        commissions = db.exec(
            select(CommissionLog).where(
                CommissionLog.dealer_id == dealer_owner_user_id,
        ...
        dealer_profile = db.get(DealerProfile, dealer_profile_id)
        ...
        chargebacks = db.exec(
            select(Chargeback).where(
                Chargeback.dealer_id == dealer_owner_user_id,
        ...
        logger.info(
            ...
            dealer_id=dealer_profile_id,
            dealer_owner_user_id=dealer_owner_user_id,
            month=month,
