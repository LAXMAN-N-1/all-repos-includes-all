...
class LogisticsService:
    ...
    def create_manifest(db: Session, driver_id: int, vehicle_id: Optional[str] = None) -> Manifest:
        ...
        if driver:
            ...
            if driver_user is not None:
                tenant_id = driver_user.created_by_dealer_id
                if tenant_id is None:
                    owner_profile = db.exec(
                        select(DealerProfile).where(DealerProfile.user_id == driver_user.id)
                    ).first()
                    tenant_id = owner_profile.id if owner_profile else None
        manifest = Manifest(
            tenant_id=tenant_id,
            driver_id=driver_id,
    ...
    def check_and_trigger_restock(db: Session, station_id: int, threshold: int = 2):
        ...
        if ready_count < threshold:
            ...
            tenant_id = station.dealer_id if station else None
    ...
    def bundle_transfers_into_manifest(db: Session, driver_id: int, transfer_ids: List[int]) -> Manifest:
        ...
        if transfers:
            manifest.tenant_id = transfers[0].tenant_id
