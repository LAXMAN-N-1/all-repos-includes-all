...
def _is_in_window(timestamp_utc_naive: datetime, *, from_utc: Optional[datetime], to_utc: Optional[datetime]) -> bool:
    ts = normalize_utc_naive(timestamp_utc_naive)
    if ts is None:
        return False
    if from_utc is not None and ts < from_utc:
        return False
    if to_utc is not None and ts > to_utc:
        return False
    return True
...
class _ActivityEvent:
    ...
    def __post_init__(self) -> None:
        if self.timestamp_utc.tzinfo is not None:
...
class AnalyticsDashboardService:
    ...
    def build_dashboard_payload(
        ...
        *,
        ...
        now = normalize_utc_naive(now_utc) or datetime.now(UTC)
    ...
    def get_recent_activity(
        ...
        *,
        ...
        if from_utc is None and to_utc is None:
            ...
            default_from = datetime.now(UTC).replace(tzinfo=None) - timedelta(days=lookback_days)
    ...
    def build_report_sections(
        ...
        *,
        ...
        tz = resolve_timezone_or_400(timezone_name)
        now_utc = to_utc if to_utc <= datetime.now(UTC) else datetime.now(UTC)
