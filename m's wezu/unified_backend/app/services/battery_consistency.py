...
def _resolve_case_insensitive_matches(
    session: Session,
    *,
    model,
    value_column,
    ...
    rows = session.exec(
        select(model.id, value_column)
        .where(func.upper(value_column).in_(normalized_values))
        .order_by(model.id.asc())
    ).all()

    return [
        (int(row_id), normalize_battery_serial(raw_value))
...
def _get_shelf_assignment(
    ...
    *,
    ...
    serial = normalize_battery_serial(serial)
    matches = _resolve_case_insensitive_matches(
        session,
        model=ShelfBattery,
        value_column=ShelfBattery.battery_id,
        normalized_values=[serial],
    )
    if not matches:
        return None
    ...
    query = select(ShelfBattery).where(ShelfBattery.id == matches[0][0])
    if for_update:
...
def _fetch_batteries_case_insensitive(
    ...
    *,
    ...
    matches = _resolve_case_insensitive_matches(
        session,
        model=Battery,
        value_column=Battery.serial_number,
        normalized_values=serials,
    )

    ids_by_serial: dict[str, int] = {}
    for battery_id, normalized in matches:
        if normalized in ids_by_serial:
            raise HTTPException(
                status_code=409,
                detail=(
        ...
        ids_by_serial[normalized] = battery_id

    missing = sorted(set(serials) - set(ids_by_serial.keys()))
    if missing:
        raise HTTPException(status_code=400, detail=f"Batteries not found: {missing}")

    query = (
        select(Battery)
        .where(Battery.id.in_(ids_by_serial.values()))
        .order_by(Battery.id.asc())
    )
    if for_update:
        query = query.with_for_update()
    ...
    return [rows_by_id[ids_by_serial[serial]] for serial in serials]
...
def get_battery_by_serial(
    ...
    *,
    ...
    query = select(Battery).where(Battery.id == matches[0][0])
    if for_update:
        query = query.with_for_update()
