    ...
    def log_action(
        ...
        try:
            log = AuditLog(
                ...
                user_id=user_id,
                action=action,
                target_table=target_table or resource_type,
                ...
                old_value=old_value,
                new_value=new_value,
                old_state=old_state or old_value,
                new_state=new_state or new_value,
            )
            if auto_commit:
                db.add(log)
                db.commit()
            ...
            try:
                ...
                savepoint.rollback()
                logger.error("Failed to write audit log in nested transaction", exc_info=True)
