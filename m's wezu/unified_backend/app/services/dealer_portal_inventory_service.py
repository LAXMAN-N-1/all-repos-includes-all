...
from app.models.battery import Battery, BatteryStatus, BatteryHealth, BatteryLifecycleEvent, LocationType
...
class DealerPortalInventoryService:
    ...
    @staticmethod
    def _get_dealer_station_ids(
        db: Session,
        dealer_id: int,
        accessible_station_ids: Optional[List[int]] = None,
    ) -> List[int]:
        """Get all station IDs belonging to a dealer.

        Checks both Station.dealer_id (FK to dealer_profiles) and
        Station.owner_id (FK to users via DealerProfile.user_id) so that
        stations created with either ownership field are included.
        """
        dp = db.get(DealerProfile, dealer_id)
        conditions = [Station.dealer_id == dealer_id]
        if dp:
            conditions.append(Station.owner_id == dp.user_id)
        stations = db.exec(
            select(Station.id).where(or_(*conditions))
        ).all()
        station_ids = list({int(sid) for sid in stations})
        if accessible_station_ids is None:
            return station_ids
        allowed = {int(sid) for sid in accessible_station_ids}
        return [sid for sid in station_ids if sid in allowed]
    ...
    def _get_station_map(
        ...
        if dp:
            conditions.append(Station.owner_id == dp.user_id)
        allowed = set(accessible_station_ids) if accessible_station_ids is not None else None
        stations = db.exec(
            select(Station).where(or_(*conditions))
        ).all()
        return {s.id: s.name for s in stations if allowed is None or s.id in allowed}

    @staticmethod
    def _get_all_dealer_battery_ids(db: Session, station_ids: List[int]) -> List[int]:
        """Return all battery IDs for the given stations.

        Unions two authoritative sources:
        - Battery.station_id (set by ORM event listener on write)
        - StationSlot.battery_id (physical slot occupancy — dealer portal source of truth)
        """
        if not station_ids:
            return []
        by_station = set(db.exec(
            select(Battery.id).where(Battery.station_id.in_(station_ids))
        ).all())
        slotted = set(db.exec(
        ...
        return list(by_station | slotted)
    ...
    def get_inventory(
        ...
        all_station_ids = svc._get_dealer_station_ids(db, dealer_id, accessible_station_ids)
        station_map = svc._get_station_map(db, dealer_id, all_station_ids)
        ...
        battery_ids = svc._get_all_dealer_battery_ids(db, all_station_ids)
        if not battery_ids:
            return {
                "batteries": [],
                "pagination": {"page": 1, "limit": limit, "total": 0, "total_pages": 0, "has_next_page": False, "has_prev_page": False},
        ...
        # Base query
        query = select(Battery).where(Battery.id.in_(battery_ids))
        ...
        if stations:
            ...
            if valid_stn_ids:
                station_battery_ids = svc._get_all_dealer_battery_ids(db, valid_stn_ids)
                query = query.where(Battery.id.in_(station_battery_ids))
        ...
        summary = svc._compute_summary(db, battery_ids)
    ...
    def get_metrics(
        ...
        svc = DealerPortalInventoryService
        station_ids = svc._get_dealer_station_ids(db, dealer_id, accessible_station_ids)

        if not station_ids:
            return svc._empty_metrics()

        battery_ids = svc._get_all_dealer_battery_ids(db, station_ids)
        if not battery_ids:
            return svc._empty_metrics()

        # Current counts by status
        counts = svc._status_counts(db, battery_ids)
        ...
        # Health distribution
        health_dist = svc._health_distribution(db, battery_ids)

        # Average health
        avg_health_result = db.exec(
            select(func.avg(Battery.health_percentage))
            .where(Battery.id.in_(battery_ids))
        ).one()
        ...
        trends = svc._compute_trends(db, battery_ids, counts)
    ...
    def get_models(
        ...
        batteries = db.exec(
            select(Battery).where(Battery.id.in_(battery_ids))
        ).all()
    ...
    def update_battery_status(
        ...
        svc = DealerPortalInventoryService
        station_ids = svc._get_dealer_station_ids(db, dealer_id, accessible_station_ids)
        all_battery_ids = svc._get_all_dealer_battery_ids(db, station_ids)

        battery = db.get(Battery, battery_id)
        if not battery or battery.id not in all_battery_ids:
    ...
    def search_inventory(
        ...
        if stations:
            ...
            if valid_ids:
                scoped_ids = svc._get_all_dealer_battery_ids(db, valid_ids)
                query = query.where(Battery.id.in_(scoped_ids))
    ...
    def bulk_update_status(
        ...
        svc = DealerPortalInventoryService
        station_ids = svc._get_dealer_station_ids(db, dealer_id, accessible_station_ids)
        all_dealer_battery_ids = set(svc._get_all_dealer_battery_ids(db, station_ids))
        ...
        for bid in battery_ids:
            ...
            if not battery or battery.id not in all_dealer_battery_ids:
    ...
    def get_trends(
        ...
        current_counts = svc._status_counts(db, battery_ids)
    ...
    @staticmethod
    def _status_counts(db: Session, battery_ids: List[int]) -> Dict[str, int]:
        """Count batteries by status for the given battery ID list."""
        results = db.exec(
    ...
    @staticmethod
    def _health_distribution(db: Session, battery_ids: List[int]) -> Dict[str, Dict]:
        """Compute health distribution across given batteries."""
        batteries = db.exec(
    ...
    @staticmethod
    def _compute_summary(db: Session, battery_ids: List[int]) -> Dict[str, Any]:
    ...
    @staticmethod
    def _compute_trends(db: Session, battery_ids: List[int], current_counts: Dict[str, int]) -> Dict[str, Dict]:
