...
class RentalService:
    ...
    def create_rental_payment_order(db: Session, rental_id: int, user_id: int) -> dict:
        ...
        receipt = f"rental_{rental.id}_{int(datetime.now(UTC).timestamp())}"
    ...
    def return_battery_for_user(
        ...
        *,
        ...
        rental = db.exec(
            select(Rental)
            .where(Rental.id == rental_id, Rental.user_id == user_id)
            .with_for_update()
        ).first()
        if not rental:
            raise HTTPException(status_code=404, detail="Rental not found")
        ...
        return RentalService.return_battery(db, rental_id, station_id)
    ...
    def review_extension(
        ...
        *,
        ...
        extension.approved_at = datetime.now(UTC)
    ...
    def review_pause(
        ...
        *,
        ...
        pause.approved_at = datetime.now(UTC)
