...
class StorageService:
    ...
    def _sanitize_prefix(prefix: Optional[str]) -> str:
        ...
        safe_parts = [part for part in raw.split("/") if part and part not in {".", ".."}]
        return "/".join(safe_parts) or "global/shared"

    @staticmethod
    def _build_storage_directory(directory: str, tenant_prefix: Optional[str]) -> str:
        safe_dir = StorageService._sanitize_directory(directory)
        safe_prefix = StorageService._sanitize_prefix(tenant_prefix)
        return f"{safe_prefix}/{safe_dir}"
    ...
    async def upload_file(
        ...
        if StorageService._should_use_s3():
            return StorageService.upload_s3(file, directory, tenant_prefix=tenant_prefix)
        return await StorageService.upload_local(file, directory, tenant_prefix=tenant_prefix)

    @staticmethod
    async def upload_local(file: UploadFile, directory: str, tenant_prefix: Optional[str] = None) -> str:
        safe_dir = StorageService._build_storage_directory(directory, tenant_prefix)
    ...
    @staticmethod
    def upload_s3(file: UploadFile, directory: str, tenant_prefix: Optional[str] = None) -> str:
        s3 = boto3.client(
            "s3",
        ...
        try:
            return s3.generate_presigned_url(
                "get_object",
                Params={"Bucket": settings.AWS_BUCKET_NAME, "Key": file_path},
                ExpiresIn=3600,
            )
        except Exception:
            logger.exception("S3 presign failed for %s", file_path)
            raise
