...
class WorkflowAutomationService:
    ...
    @staticmethod
    def notify_logistics_order_approval_requested(
        db: Session,
        *,
        order_id: str,
        dealer_id: int,
        ...
        destination_text = (destination or "").strip() or "assigned destination"
        message = (
            f"Dealer #{dealer_id} requested order {order_id} for {requested_units} battery unit(s) "
            f"to {destination_text}. Admin approval is required."
        )
        return WorkflowAutomationService._notify_internal_ops(
            db,
            title="Order Approval Required",
            message=message,
            notification_type="logistics_order_approval_requested",
            channels=("push", "email"),
            category="operational",
            dedupe_window_minutes=10,
            exclude_user_ids=[requester_user_id] if requester_user_id is not None else None,
            app_scope=WorkflowAutomationService.LOGISTICS_APP_SCOPE,
        )

    @staticmethod
    def notify_logistics_order_approval_decided(
        db: Session,
        *,
        order_id: str,
        approved: bool,
        ...
        state_text = "approved" if approved else "rejected"
        warehouse_text = f" and assigned to warehouse #{warehouse_id}" if approved and warehouse_id is not None else ""
        notes_text = f" Notes: {notes}" if notes else ""

        sent_any = False
        if requester_user_id is not None:
            sent_any = WorkflowAutomationService._notify_channels(
                db,
                user_id=requester_user_id,
                title=f"Order {state_text.title()}",
                message=f"Order {order_id} has been {state_text}{warehouse_text}.{notes_text}",
                notification_type=f"logistics_order_{state_text}",
                channels=("push", "email"),
                category="transactional",
                dedupe_window_minutes=10,
                app_scope=WorkflowAutomationService.LOGISTICS_APP_SCOPE,
        ...
        sent_ops = WorkflowAutomationService._notify_internal_ops(
            ...
            title=f"Order {state_text.title()}",
            message=(
                f"Order {order_id} for dealer #{dealer_id or 'unknown'} has been {state_text}{warehouse_text}.{notes_text}"
            ),
            notification_type=f"logistics_order_{state_text}_ops",
            channels=("push", "email"),
            category="operational",
            dedupe_window_minutes=10,
            exclude_user_ids=[requester_user_id] if requester_user_id is not None else None,
