from sqlmodel import Session, select
from app.models.audit_log import SecurityEvent
from datetime import datetime, UTC
from typing import List, Optional
import logging

logger = logging.getLogger(__name__)

class SecurityService:
    @staticmethod
    def log_event(
        db: Session,
        event_type: str,
        severity: str = "medium",
        details: str = None,
        source_ip: str = None,
        user_id: Optional[int] = None
    ) -> SecurityEvent:
        """
        Log a security-related event.
        Severity levels: low, medium, high, critical
        """
        event = SecurityEvent(
            event_type=event_type,
            severity=severity,
            details=details,
            source_ip=source_ip,
            user_id=user_id,
            timestamp=datetime.now(UTC),
            is_resolved=False
        )
        db.add(event)
        db.commit()
        db.refresh(event)
        
        if severity in ["high", "critical"]:
            logger.warning(f"HIGH SEVERITY SECURITY EVENT: {event_type} (User: {user_id}, IP: {source_ip})")
            # In a real app, this might trigger an email/SMS/Webhook to the security team
            
        return event

    @staticmethod
    def get_events(
        db: Session,
        skip: int = 0,
        limit: int = 100,
        unresolved_only: bool = False,
        severity: Optional[str] = None
    ) -> List[SecurityEvent]:
        statement = select(SecurityEvent)
        if unresolved_only:
            statement = statement.where(SecurityEvent.is_resolved == False)
        if severity:
            statement = statement.where(SecurityEvent.severity == severity)
        
        statement = statement.offset(skip).limit(limit).order_by(SecurityEvent.timestamp.desc())
        return db.exec(statement).all()

    @staticmethod
    def resolve_event(db: Session, event_id: int) -> Optional[SecurityEvent]:
        event = db.get(SecurityEvent, event_id)
        if event:
            event.is_resolved = True
            db.add(event)
            db.commit()
            db.refresh(event)
        return event

    @staticmethod
    def get_event_stats(db: Session):
        """Get summary statistics for security events"""
        from sqlalchemy import func
        
        counts = db.exec(
            select(SecurityEvent.severity, func.count(SecurityEvent.id))
            .group_by(SecurityEvent.severity)
        ).all()
        
        unresolved_count = db.exec(
            select(func.count(SecurityEvent.id))
            .where(SecurityEvent.is_resolved == False)
        ).first()
        
        return {
            "severity_counts": dict(counts),
            "unresolved_count": unresolved_count or 0
        }

    # ── Security-question helpers ──────────────────────────────────────────
    _DEFAULT_QUESTIONS: list[str] = [
        "What was the name of your first pet?",
        "What city were you born in?",
        "What is your mother's maiden name?",
        "What was the name of your first school?",
        "What is the make of your first car?",
    ]

    @staticmethod
    def get_available_questions(db: Session) -> list[dict]:
        """Return active security questions from persistent table."""
        from app.models.security_question import SecurityQuestion

        questions = db.exec(
            select(SecurityQuestion).where(SecurityQuestion.is_active == True)  # noqa: E712
        ).all()
        if not questions:
            for question_text in SecurityService._DEFAULT_QUESTIONS:
                db.add(SecurityQuestion(question_text=question_text, is_active=True))
            db.commit()
            questions = db.exec(
                select(SecurityQuestion).where(SecurityQuestion.is_active == True)  # noqa: E712
            ).all()

        return [
            {"id": question.id, "question_text": question.question_text}
            for question in questions
            if question.id is not None
        ]

    @staticmethod
    def set_user_security_question(
        db: Session,
        user_id: int,
        question_id: int,
        answer: str,
    ) -> None:
        """Store a hashed security-question answer for a user."""
        from app.models.security_question import SecurityQuestion, UserSecurityQuestion
        from app.core.security import get_password_hash

        question = db.get(SecurityQuestion, question_id)
        if not question or not question.is_active:
            raise ValueError(f"Security question {question_id} is invalid")

        existing = db.exec(
            select(UserSecurityQuestion).where(UserSecurityQuestion.user_id == user_id)
        ).first()
        if not existing:
            existing = UserSecurityQuestion(
                user_id=user_id,
                question_id=question_id,
                hashed_answer=get_password_hash(answer.strip().lower()),
            )
        else:
            existing.question_id = question_id
            existing.hashed_answer = get_password_hash(answer.strip().lower())
            existing.updated_at = datetime.now(UTC)

        db.add(existing)
        db.commit()

    @staticmethod
    def verify_security_answer(db: Session, user_id: int, answer: str) -> bool:
        """Verify a user's security-question answer."""
        from app.models.security_question import UserSecurityQuestion
        from app.core.security import verify_password

        entry = db.exec(
            select(UserSecurityQuestion).where(UserSecurityQuestion.user_id == user_id)
        ).first()
        if not entry:
            return False

        return verify_password(answer.strip().lower(), entry.hashed_answer)
