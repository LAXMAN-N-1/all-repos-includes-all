...
class DeliveryOrderResponse(DeliveryOrderBase):
    ...
    def resolve_public_urls(self):
        from app.core.public_url import to_public_url
        if self.proof_of_delivery_url:
            self.proof_of_delivery_url = to_public_url(self.proof_of_delivery_url)
        if getattr(self, "customer_signature_url", None):
            self.customer_signature_url = to_public_url(self.customer_signature_url)
        return self
