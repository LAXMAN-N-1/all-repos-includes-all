...
def _status_from_numeric_code(code_value: int) -> Optional[str]:
    code_map = {
        0: "PENDING_ADMIN_APPROVAL",
        1: "APPROVED",
        2: "ASSIGNED_TO_WAREHOUSE",
        3: "OUT_FOR_DELIVERY",
        4: "DELIVERED",
        5: "REJECTED",
        6: "REJECTED",
...
class OrderCreate(BaseModel):
    ...
    status: Optional[str] = "PENDING_ADMIN_APPROVAL"
    ...
    assigned_battery_ids: Optional[List[str]] = None
...
class DeliveryApproveAction(BaseModel):
    warehouse_id: Optional[int] = None
    notes: Optional[str] = None


class DeliveryRejectAction(BaseModel):
    reason: Optional[str] = None


class DeliveryDispatchAction(BaseModel):
    driver_id: int
    warehouse_operator_id: Optional[int] = None
    notes: Optional[str] = None


class DeliveryCompleteAction(BaseModel):
...
class OrderBatteryAssignment(BaseModel):
