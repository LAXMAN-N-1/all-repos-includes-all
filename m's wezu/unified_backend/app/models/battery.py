...
from sqlalchemy import Column, Index, JSON, Enum as SAEnum, func, event
...
class Battery(SQLModel, table=True):
    ...
    status: BatteryStatus = Field(
        ...
        sa_column=SQLColumn(SAEnum(BatteryStatus, values_callable=lambda obj: [e.value for e in obj]), index=True)
    )
    ...
    location_type: LocationType = Field(
        ...
        sa_column=SQLColumn(SAEnum(LocationType, values_callable=lambda obj: [e.value for e in obj]))
    )
...
Index(
    "ix_batteries_serial_number_upper",
    func.upper(Battery.__table__.c.serial_number),
    unique=False,
)


def _sync_battery_location(target: Battery) -> None:
    """Enforce invariant: station_id and (location_type=STATION, location_id) must always agree.

    Priority rules (applied in order):
    1. station_id set → it is authoritative for station placement;
       force location_type=STATION and location_id=station_id.
    2. location_type=STATION + location_id set, station_id absent →
       back-fill station_id from location_id.
    3. location_type=STATION + location_id absent, station_id set →
       repair location_id from station_id.
    4. Not at a station → station_id must be NULL; clear it.
    """
    loc_type_val = (
        target.location_type.value
        if isinstance(target.location_type, LocationType)
        else str(target.location_type or "")
    )
    at_station = loc_type_val == LocationType.STATION.value

    if target.station_id is not None:
        # Rule 1: station_id is authoritative
        target.location_type = LocationType.STATION
        target.location_id = target.station_id
    elif at_station and target.location_id is not None:
        # Rule 2: polymorphic fields are authoritative
        target.station_id = int(target.location_id)
    elif at_station and target.location_id is None and target.station_id is not None:
        # Rule 3: repair missing location_id
        target.location_id = target.station_id
    else:
        # Rule 4: not at a station — station_id must be NULL
        target.station_id = None


@event.listens_for(Battery, "before_insert")
@event.listens_for(Battery, "before_update")
def _enforce_battery_location_invariant(mapper, connection, target: Battery) -> None:
    _sync_battery_location(target)
