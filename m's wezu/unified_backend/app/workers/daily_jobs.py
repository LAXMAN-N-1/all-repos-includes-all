...
def idempotency_key_cleanup():
    """Delete expired idempotency keys daily to keep table size bounded."""
    logger.info("Starting idempotency key cleanup...")
    execution = create_job_execution("daily_idempotency_key_cleanup")
    try:
        from app.services.idempotency_service import cleanup_expired_idempotency_keys

        with Session(engine) as session:
            deleted_rows = cleanup_expired_idempotency_keys(session)

        result = {"deleted_rows": deleted_rows, "timestamp": datetime.now(UTC).isoformat()}
        logger.info("Idempotency key cleanup completed: deleted=%s", deleted_rows)
        complete_job_execution(execution.execution_id, "COMPLETED", result)
    except Exception as exc:
        logger.error("Idempotency key cleanup failed: %s", exc, exc_info=True)
        complete_job_execution(execution.execution_id, "FAILED", error=str(exc))
