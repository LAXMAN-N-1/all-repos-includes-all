...
def create_station(
    ...
    dealer = None
    if request.dealer_id is not None:
        dealer = db.get(DealerProfile, request.dealer_id)
        if not dealer:
    ...
    station = Station(
        ...
        tenant_id=dealer.tenant_id if dealer else None,
        owner_id=dealer.user_id if dealer else None,
        ...
        status=request.status or StationStatus.ACTIVE,
