...
router = APIRouter(
    dependencies=[
        Depends(
            deps.require_global_permission(
...
def _mark_legacy_dashboard_adapter(response: Response, successor_path: str) -> None:
...
async def get_dashboard_summary(
    ...
    _mark_legacy_dashboard_adapter(response, "/api/v1/admin/analytics/dashboard")
...
@router.get("/trend", deprecated=True)
async def get_dashboard_trend(
    ...
    _mark_legacy_dashboard_adapter(response, "/api/v1/admin/analytics/trends")
...
@router.get("/station-health", deprecated=True)
async def get_station_health(
    response: Response,
    db: Session = Depends(deps.get_db),
):
    _mark_legacy_dashboard_adapter(response, "/api/v1/admin/analytics/battery-health-distribution")
...
@router.get("/activity-feed", deprecated=True)
async def get_dashboard_activity_feed(
    response: Response,
    limit: int = Query(10, ge=1, le=100),
    db: Session = Depends(deps.get_db),
):
    _mark_legacy_dashboard_adapter(response, "/api/v1/admin/analytics/recent-activity")
...
@router.get("/top-stations", deprecated=True)
async def get_dashboard_top_stations(
    response: Response,
    limit: int = Query(5, ge=1, le=50),
    db: Session = Depends(deps.get_db),
):
    _mark_legacy_dashboard_adapter(response, "/api/v1/admin/analytics/top-stations")
