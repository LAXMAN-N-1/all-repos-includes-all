        ...
        results.append(
            {
                ...
                "dealer_owner_user_id": dealer.user_id,
...
def dealer_list_settlements(
    ...
    dealer_profile_id, _ = SettlementService.resolve_dealer_scope_ids(db, current_user.id)
    return db.exec(
        select(Settlement)
        .where(Settlement.dealer_id == dealer_profile_id)
...
def dealer_settlement_details(
    ...
    dealer_profile_id, _ = SettlementService.resolve_dealer_scope_ids(db, current_user.id)
    if not settlement or settlement.dealer_id != dealer_profile_id:
