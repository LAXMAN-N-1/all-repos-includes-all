...
def _find_manifests_by_normalized_id(
    ...
    *,
    ...
    if tenant_context.scope == "tenant" and tenant_context.tenant_id is not None:
        query = query.where(Manifest.tenant_id == int(tenant_context.tenant_id))
    if for_update:
...
def create_manifest(
    ...
    if _find_manifests_by_normalized_id(session, normalized_manifest_id, tenant_context):
        raise HTTPException(
            status_code=409,
    ...
    manifest = _load_manifest_by_id(session, normalized_manifest_id, tenant_context)
