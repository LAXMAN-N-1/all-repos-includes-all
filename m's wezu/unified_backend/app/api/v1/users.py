...
async def _validate_image_payload(file: UploadFile) -> None:
    ...
    await file.seek(0)
    magic_prefixes = IMAGE_MAGIC_PREFIXES.get(content_type, ())
...
async def upload_profile_picture(
    ...
    await _validate_image_payload(file)
...
@router.get("/me/dashboard-summary", response_model=DashboardSummaryResponse, deprecated=True)
async def get_my_dashboard_summary(
    ...
    from app.api.v1.customer_dashboard import _build_customer_dashboard_stats
    ...
    total_spent_this_month = float(
        db.exec(
            select(func.coalesce(func.sum(Rental.total_amount), 0.0)).where(
    ...
    lifetime_rentals_count = int(
        db.exec(select(func.count(Rental.id)).where(Rental.user_id == current_user.id)).one() or 0
    )

    canonical = AnalyticsService.get_customer_dashboard(current_user.id, db) or {}
    canonical_stats = _build_customer_dashboard_stats(db=db, user_id=current_user.id)
    return {
        "total_spent_this_month": round(total_spent_this_month, 2),
        "active_rentals_count": active_rentals_count,
        "lifetime_rentals_count": lifetime_rentals_count,
        "membership_tier": str(canonical.get("membership_tier") or "basic"),
        "wallet_balance": float(canonical_stats.get("wallet_balance") or 0.0),
        "carbon_saved_kg": float(canonical_stats.get("carbon_saved_kg") or 0.0),
        "quick_stats": {
            "active_rental_id": canonical.get("active_rental_id"),
            "points_balance": canonical.get("points_balance", 0),
        },
    }
...
async def get_my_sessions(
    ...
    raise HTTPException(
        ...
        detail="Session management is owned by Supabase in this release",
    )
