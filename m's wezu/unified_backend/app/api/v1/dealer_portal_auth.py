...
from app.schemas.auth import (
    ...
    DealerPortalLoginResponse,
    DealerRegistrationStatusResponse,
...
@router.post("/login", response_model=DealerPortalLoginResponse)
...
async def dealer_login(
    ...
    if not user or not user.hashed_password:
        logger.warning("Dealer login failed: user not found or password missing identifier=%s", identifier)
        raise HTTPException(status_code=401, detail="Invalid credentials")

    # Brute-force protection
    now_utc = utcnow()
    locked_until_utc = ensure_utc_aware(user.locked_until)
    if locked_until_utc and locked_until_utc > now_utc:
        remaining = int((locked_until_utc - now_utc).total_seconds() / 60) + 1
        logger.warning("Dealer login blocked: account locked user_id=%s remaining_minutes=%s", user.id, remaining)
        raise HTTPException(
            status_code=423,
    ...
    if not verify_password(login_data.password, user.hashed_password):
        ...
        if new_count >= MAX_LOGIN_ATTEMPTS:
            ...
            db.commit()
            logger.warning("Dealer login blocked: lockout threshold reached user_id=%s", user.id)
            raise HTTPException(
                status_code=423,
                detail=f"Account locked for {LOCKOUT_MINUTES} minutes after {MAX_LOGIN_ATTEMPTS} failed attempts."
            )
        db.commit()
        logger.warning("Dealer login failed: invalid password user_id=%s", user.id)
        raise HTTPException(status_code=401, detail="Invalid credentials")

    if user.status not in [UserStatus.ACTIVE, UserStatus.PENDING_VERIFICATION]:
        if user.status == UserStatus.PENDING:
            logger.warning("Dealer login blocked: account pending activation user_id=%s", user.id)
            raise HTTPException(status_code=403, detail="Account is pending activation. Check your email for the invite link.")
        logger.warning("Dealer login blocked: inactive/suspended account user_id=%s status=%s", user.id, user.status)
        raise HTTPException(status_code=403, detail="Account is inactive or suspended")

    dealer_profile = deps.get_dealer_profile_for_user(db, user)
    if not dealer_profile:
        logger.warning("Dealer login blocked: missing dealer profile user_id=%s", user.id)
    ...
    # Generate refresh token and bind session first for stable access SID.
...
@router.get("/register/status", response_model=DealerRegistrationStatusResponse)
...
def refresh_token(
    ...
    """Refresh token using canonical session-validation + rotation pipeline."""
    access_token, new_refresh, user = AuthService.rotate_refresh_session_tokens(
        db=db,
...
def validate_invite_token(
    ...
    invite_expires_at_utc = ensure_utc_aware(user.invite_token_expires)
    if invite_expires_at_utc and invite_expires_at_utc < utcnow():
        return {
...
@router.post("/activate/{token}", response_model=DealerPortalLoginResponse)
...
@router.post("/force-change-password", response_model=ForcePasswordChangeResponse)
