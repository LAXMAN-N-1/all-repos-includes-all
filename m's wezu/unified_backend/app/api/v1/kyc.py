...
DOCUMENT_MIME_MAGIC_PREFIXES: dict[str, tuple[bytes, ...]] = {
    "image/jpeg": (b"\xff\xd8\xff",),
    "image/png": (b"\x89PNG",),
    "application/pdf": (b"%PDF",),
}

VIDEO_ALLOWED_TYPES: set[str] = {
    "video/mp4",
    "video/webm",
    "video/quicktime",
}

VIDEO_MAGIC_PREFIXES: tuple[bytes, ...] = (b"\x00\x00\x00\x18ftyp", b"\x00\x00\x00\x1cftyp", b"\x1aE\xdf\xa3")


def _validate_file_content(file: UploadFile, *, allowed_content_types: set[str], magic_prefixes: tuple[bytes, ...] | None = None) -> None:
    content_type = (file.content_type or "").strip().lower()
    if content_type not in allowed_content_types:
        raise HTTPException(status_code=400, detail="Unsupported file type")

    if magic_prefixes is None:
        magic_prefixes = tuple(
            prefix
            for allowed_type in allowed_content_types
            for prefix in DOCUMENT_MIME_MAGIC_PREFIXES.get(allowed_type, ())
        )
...
def _safe_upload_path(upload_dir: str, prefix: str, suffix: str = ".bin") -> str:
    return os.path.join(upload_dir, f"{prefix}_{uuid4().hex}{suffix}")


def _kyc_hmac_secret() -> str:
    secret = (settings.KYC_HMAC_SECRET or "").strip()
    if secret:
        return secret
    if (settings.ENVIRONMENT or "").strip().lower() == "production":
        raise HTTPException(status_code=503, detail="kyc_verification_unavailable")
    return settings.SECRET_KEY


def _hmac_pii(value: str) -> str:
    return hmac.new(
        _kyc_hmac_secret().encode("utf-8"),
        value.encode("utf-8"),
        hashlib.sha256,
    ).hexdigest()
...
async def submit_kyc_document(
    ...
    _validate_file_content(
        file,
        allowed_content_types=set(DOCUMENT_MIME_MAGIC_PREFIXES.keys()),
    )

    file_path = _safe_upload_path(upload_dir, document_type)
...
async def verify_aadhaar(
    ...
    # 2. Store deterministically via HMAC (lookup-safe and resistant to rainbow tables).
    encrypted_val = _hmac_pii(aadhaar_number)
...
async def verify_pan(
    ...
    encrypted_val = _hmac_pii(pan_number)
...
async def upload_video_kyc(
    ...
    _validate_file_content(
        file,
        allowed_content_types=VIDEO_ALLOWED_TYPES,
        magic_prefixes=VIDEO_MAGIC_PREFIXES,
    )
    file_path = _safe_upload_path(upload_dir, "video_kyc")
...
async def verify_utility_bill(
    ...
    file_path = _safe_upload_path(upload_dir, "utility_bill")
