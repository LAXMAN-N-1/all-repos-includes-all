...
router = APIRouter(
    dependencies=[Depends(deps.require_tenant_context(allow_global=False))]
)
...
def get_dealer_transactions(
    ...
    # FastAPI parses bare datetime query params as naive (no tzinfo). Coerce to
    # UTC before any DB comparison so we never hit offset-naive vs aware errors.
    ...
    if start_date is not None:
        query = query.where(Rental.created_at >= start_date)
    if end_date is not None:
        query = query.where(Rental.created_at <= end_date)
    if amount_min is not None:
        query = query.where(func.coalesce(Rental.total_amount, 0) >= amount_min)
    if amount_max is not None:
        query = query.where(func.coalesce(Rental.total_amount, 0) <= amount_max)
    if status:
