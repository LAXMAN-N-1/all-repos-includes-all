...
def _get_dealer(db: Session, current_user: User) -> DealerProfile:
    dealer = deps.get_dealer_profile_for_user(db, current_user)
    if not dealer:
        raise HTTPException(status_code=403, detail="Not a dealer")
    return dealer


def _dealer_inventory_scope(db: Session, current_user: User) -> tuple[DealerProfile, list[int]]:
    dealer = _get_dealer(db, current_user)
    return dealer, accessible_station_ids_for_user(db, current_user, dealer)
...
def get_dealer_inventory(
    ...
    """
    dealer, station_ids = _dealer_inventory_scope(db, current_user)
    dealer_id = dealer.id
    ...
    result = svc.get_inventory(
        ...
        accessible_station_ids=station_ids,
    )
...
def get_inventory_metrics(
    ...
    dealer, station_ids = _dealer_inventory_scope(db, current_user)
    dealer_id = dealer.id
    data = svc.get_metrics(db, dealer_id, accessible_station_ids=station_ids)
...
def get_health_analytics(
    ...
    dealer, station_ids = _dealer_inventory_scope(db, current_user)
    dealer_id = dealer.id
    data = svc.get_health_analytics(db, dealer_id, accessible_station_ids=station_ids)
...
def get_inventory_models(
    ...
    dealer, station_ids = _dealer_inventory_scope(db, current_user)
    dealer_id = dealer.id
    data = svc.get_models(db, dealer_id, accessible_station_ids=station_ids)
...
def create_stock_request(
    ...
    try:
        data = svc.create_stock_request(
            ...
            notes=body.notes,
            user_id=current_user.id,
            tenant_id=dealer.tenant_id or dealer.id,
            accessible_station_ids=station_ids,
        )
    except ValueError as e:
        if str(e) == "Station not found":
