...
@router.get("/{serial}/trace", response_model=DataResponse[dict])
def get_battery_trace(
    serial: str,
    limit: int = Query(default=200, ge=1, le=1000),
    current_user: User = Depends(deps.require_internal_operator),
    session: Session = Depends(deps.get_db),
):
    """Cross-order trace for one battery serial (order legs + inventory + lifecycle)."""
    del current_user
    normalized_serial = str(serial or "").strip().upper()
    if not normalized_serial:
        raise HTTPException(status_code=400, detail="battery serial is required")

    battery = session.exec(
        select(Battery).where(func.upper(Battery.serial_number) == normalized_serial).limit(1)
    ).first()
    if battery is None:
        raise HTTPException(status_code=404, detail="Battery not found")

    leg_links = session.exec(
        select(OrderLegBattery)
        .where(func.upper(OrderLegBattery.battery_id) == normalized_serial)
        .order_by(OrderLegBattery.recorded_at.desc(), OrderLegBattery.id.desc())
    ).all()
    leg_ids = sorted({int(link.order_leg_id) for link in leg_links if link.order_leg_id is not None})

    legs_by_id: Dict[int, OrderLeg] = {}
    events: List[OrderLegEvent] = []
    if leg_ids:
        legs = session.exec(select(OrderLeg).where(OrderLeg.id.in_(leg_ids))).all()
        legs_by_id = {int(leg.id): leg for leg in legs if leg.id is not None}
        events = session.exec(
    ...
    for event in events:
        leg = legs_by_id.get(int(event.order_leg_id))
        timeline.append(
            {
                "source": "order_trace",
                ...
                "leg_sequence": leg.leg_sequence if leg else None,
                "leg_type": leg.leg_type if leg else None,
                ...
                "source_location_type": leg.source_location_type if leg else None,
                "source_location_id": leg.source_location_id if leg else None,
                "destination_location_type": leg.destination_location_type if leg else None,
                "destination_location_id": leg.destination_location_id if leg else None,
            }
        )
    ...
    if battery.id is not None:
        inventory_logs = session.exec(
            ...
            .where(InventoryAuditLog.battery_id == int(battery.id))
        ...
        lifecycle_events = session.exec(
            ...
            .where(BatteryLifecycleEvent.battery_id == int(battery.id))
...
def read_batteries(
    ...
    include_pagination: bool = Query(
        default=False,
        description="Compatibility flag accepted for list clients; battery list returns an array payload.",
    ),
    ...
    """
    # Backward-compat: some clients pass include_pagination on all list calls.
    # This endpoint currently returns a plain array and ignores the flag.
    _ = include_pagination
    query = select(Battery)
    
    if not current_user.is_superuser:
        if actor_context.role == "warehouse_operator":
            warehouse_ids = _assigned_warehouse_ids(session, current_user)
            if actor_context.warehouse_id is not None:
                warehouse_ids.add(int(actor_context.warehouse_id))
            if not warehouse_ids:
                return []
            query = query.where(
                Battery.location_type == "warehouse",
                Battery.location_id.in_(sorted(warehouse_ids)),
            )
