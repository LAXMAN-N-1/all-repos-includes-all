...
def create_role(
    *,
    ...
    role = role_service.create_role_record(
        db,
        role_data=role_data,
        permission_ids=role_in.permission_ids,
        permission_slugs=list(permissions_to_assign),
    )
...
def update_role(
    *,
    ...
    changes = []
    update_data = role_in.model_dump(exclude={"permissions"}, exclude_unset=True)

    if "name" in update_data and update_data["name"] != role.name:
        existing = role_service.get_role_by_name(db, update_data["name"])
        if existing and existing.id != role.id:
            raise HTTPException(status_code=400, detail="Role name already exists")

    for field_name, new_value in update_data.items():
        if field_name == "parent_role_id":
            old_value = role.parent_id
        else:
            old_value = getattr(role, field_name, None)
        if new_value != old_value:
            if field_name == "description":
                changes.append("Description changed")
            elif field_name == "parent_role_id":
                changes.append(f"Parent Role ID: {old_value} -> {new_value}")
            else:
                changes.append(f"{field_name.title()}: {old_value} -> {new_value}")

    role = role_service.update_role_fields(
        db,
        role_id,
        update_data=update_data,
        permission_slugs=role_in.permissions if role_in.permissions is not None else None,
    )
    if not role:
        raise HTTPException(status_code=404, detail="Role not found")

    # 3. Session Invalidation (Security)
    if role_in.permissions is not None:
        if role_in.permissions:
            changes.append(f"Permissions set to: {', '.join(role_in.permissions)}")
        else:
            changes.append("Permissions set to: (none)")
...
def get_role_detail(
    ...
    user_count_map, _ = role_service.get_role_user_counts(db, [role_id])
...
def delete_role(
    ...
    # 3. Soft Delete
    role_service.soft_delete_role(db, role_id)
