...
def list_dealers(
    ...
    rows = db.exec(
        query.order_by(DealerProfile.created_at.desc(), DealerProfile.id.desc())
        .offset(actual_skip)
        .limit(limit + 1)
    ).all()
    ...
    next_cursor = None
    if has_more:
        next_cursor = encode_cursor({"offset": int(actual_skip) + len(results)})
    return {
        "dealers": results,
        "total_count": total,
        "meta": {
...
def create_dealer(
    ...
    """Admin-initiated dealer creation backed by Supabase Auth and tenant linkage."""
    existing = db.exec(select(User).where(User.email == data.email)).first()
    if existing:
        raise HTTPException(400, "User with this email already exists")

    role = db.exec(select(Role).where(func.lower(Role.name) == "dealer_owner")).first()
    if not role:
        role = db.exec(select(Role).where(func.lower(Role.name) == "dealer")).first()
    if not role:
        raise HTTPException(status_code=400, detail="Dealer role not found")
    ...
    try:
        ...
        tenant = TenantService.ensure_dealer_tenant(db, dp)
        TenantService.ensure_membership(
            db,
            tenant_id=tenant.id,
            user_id=user.id,
            scope="tenant_owner",
...
# ─── Dealer Inventory (Admin View) ─────────────────────────────────────────

@router.get("/{dealer_id}/inventory/metrics")
def get_dealer_inventory_metrics(
    dealer_id: int,
    db: Session = Depends(deps.get_db),
    current_user: Any = Depends(deps.get_current_active_admin),
) -> Any:
    """Admin: KPI metrics for a specific dealer's inventory."""
    dp = db.get(DealerProfile, dealer_id)
    if not dp:
        raise HTTPException(404, "Dealer not found")
    data = DealerPortalInventoryService.get_metrics(db, dealer_id, accessible_station_ids=None)
...
def get_dealer_inventory(
    ...
    """Admin: paginated battery inventory for a specific dealer."""
    dp = db.get(DealerProfile, dealer_id)
    if not dp:
        raise HTTPException(404, "Dealer not found")
    result = DealerPortalInventoryService.get_inventory(
        db=db,
        dealer_id=dealer_id,
        page=page,
        limit=limit,
