...
from app.api.admin.users import (
    AdminUserCreateRequest as UnifiedAdminUserCreateRequest,
    admin_create_user as create_user_via_supabase,
)
...
async def admin_create_user(
    payload: UnifiedAdminUserCreateRequest,
    db: Session = Depends(deps.get_db),
    current_user: User = Depends(deps.get_current_active_admin),
):
    return create_user_via_supabase(payload=payload, db=db, current_user=current_user)
