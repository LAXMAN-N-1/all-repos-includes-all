...
if sys.version_info < (3, 11):
    raise RuntimeError(
        "Tests require Python >= 3.11 for this repository. "
...
# Use PostgreSQL for tests when explicitly configured.
# Otherwise default to an in-memory SQLite database so schema always matches
# the current models and cannot drift across test runs.
...
# For SQLite, rebuild the schema from metadata on every test run so
# add-only model changes are reflected immediately.
if _is_sqlite:
