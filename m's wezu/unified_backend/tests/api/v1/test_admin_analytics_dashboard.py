...
def _logistics_user_with_global_analytics(session: Session) -> User:
    role = session.exec(select(Role).where(Role.name == "logistics_manager")).first()
    if role is None:
        role = Role(name="logistics_manager", is_active=True)
        session.add(role)
        session.commit()
        session.refresh(role)

    permission = session.exec(select(Permission).where(Permission.slug == "analytics:view:global")).first()
    if permission is None:
        permission = Permission(slug="analytics:view:global", module="analytics", action="view", scope="global")
    ...
    user = User(
        email="logistics-analytics@test.com",
...
def test_logistics_manager_can_access_admin_analytics_with_global_permission(
    client: TestClient,
    session: Session,
):
    user = _logistics_user_with_global_analytics(session)
    _override_admin(client, user)
    invalidate_cache("admin-analytics")

    sample_overview = {
        "total_revenue": {"label": "Total Revenue", "value": 321},
    }
    with patch.object(
        AnalyticsService,
        "get_platform_overview",
    ...
    assert response.status_code == 200
    assert response.json()["total_revenue"]["value"] == 321
