...
def create_warehouse(session: Session, name: str, code: str, manager_id: int | None = None) -> Warehouse:
    warehouse = Warehouse(
        name=name,
        code=code,
...
class TestRequirePermission:
    ...
    def test_logistics_role_can_read_stations_without_station_permission(self, client: TestClient, session: Session):
        user = create_user(session, "logistics-reader@test.com")
        role = create_role_with_permissions(session, "logistics_manager", ["logistics:view:global"])
        assign_role(session, user, role)
        session.refresh(user)
...
class TestWarehouseBatteryFiltering:
    def test_warehouse_user_sees_only_batteries_in_assigned_warehouse(self, client: TestClient, session: Session):
        user = create_user(session, "warehouse@test.com")
        role = create_role_with_permissions(session, "warehouse_manager", ["battery:read"])
        assign_role(session, user, role)
        warehouse = create_warehouse(session, "Primary Warehouse", "WH-001")
        other_warehouse = create_warehouse(session, "Secondary Warehouse", "WH-002")
        session.add(
        ...
        create_battery(session, "STN-002-BAT", location_type="station", location_id=1)

        app = client.app
        app.dependency_overrides[deps.get_current_user] = lambda: user
        app.dependency_overrides[deps.get_actor_context] = lambda: deps.ActorContext(
        ...
        data = resp.json()
        serials = [b["serial_number"] for b in data]
        assert "WH-001-BAT" in serials
        assert "WH-002-BAT" not in serials
        assert "STN-002-BAT" not in serials
