...
class TestRouteCollisions:
    ...
    def test_route_count_sanity(self):
        ...
        routes = _all_routes()
        total = len(routes)
        # Trailing-slash mirroring doubles registered route entries.
        # Keep a broad sanity range to catch catastrophic accidental changes.
        assert 800 < total < 2600, (
            f"Route count {total} is outside expected range (800, 2600). "
            "If you intentionally added/removed many routes, update this test."
        )

    def test_no_forbidden_legacy_prefixes(self):
        """
        Standardization guard:
        - no /api/admin/*
        - no /api/v1/dealer/portal/*
        - no /api/v1/payments/admin/*
        - no /api/v1/orders/* (logistics now under /deliveries)
        """
        routes = _all_routes()
        forbidden_prefixes = (
            "/api/admin/",
        ...
        if offenders:
            pytest.fail("Forbidden legacy prefixes detected:\n  " + "\n  ".join(offenders))

    def test_no_my_scope_in_migrated_domains(self):
        """
        '/my' is forbidden in canonical migrated domains.
        This is intentionally scoped to domains included in the fast-cutover wave.
        """
        routes = _all_routes()
        migrated_domain_prefixes = (
        ...
        offenders = sorted(
            {
                f"{method} {path}"
                for method, path, _ in routes
                if any(path.startswith(prefix) for prefix in migrated_domain_prefixes)
                and "/my" in path
            }
        )
        if offenders:
            pytest.fail("Deprecated '/my' scope found in migrated domains:\n  " + "\n  ".join(offenders))
