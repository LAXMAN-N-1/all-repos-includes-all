...
async def reserve_battery(
    ...
    if existing:
        # Check if expired
        existing_end_time = ensure_utc_aware(existing.end_time)
        if existing_end_time and existing_end_time < utcnow():
...
async def get_active_reservation(
    ...
    # Auto-expire check
    reservation_end_time = ensure_utc_aware(reservation.end_time)
    if reservation_end_time and reservation_end_time < utcnow():
...
async def get_reservation_status(
    ...
    if (
        ...
        and reservation_end_time < utcnow()
    ):
        reservation.status = "EXPIRED"
        db.add(reservation)
        db.commit()
