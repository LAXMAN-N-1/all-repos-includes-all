...
async def refresh_token(
    ...
    try:
        ...
        # Use the stable DB session.id as the access-token sid so that rotations
        # never invalidate a concurrently-inflight access token.
        if old_jti:
            session = AuthService.update_user_session(db, old_jti, new_refresh_token, request)
        else:
            AuthService.revoke_session(db, refresh_data.refresh_token)
            session = AuthService.create_user_session(db, user.id, new_refresh_token, request, token_jti=new_jti)

        sid = str(session.id) if session and session.id else new_jti
...
async def send_email_verification(
    ...
    last_sent = ensure_utc_aware(getattr(current_user, 'email_verification_sent_at', None))
    now_utc = utcnow()
