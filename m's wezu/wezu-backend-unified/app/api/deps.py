...
from app.utils.datetime_utils import ensure_utc_aware, utcnow_naive
import logging
...
def _validate_local_access_token(db: Session, token: str) -> _ValidatedToken:
    ...
    if sid:
        ...
        # New tokens carry sid=str(session.id) (stable integer PK); old tokens
        # carry sid=token_jti (UUID).  Support both for a seamless transition.
        user_session = None
        try:
            session_id = int(sid)
            ...
            # sid is a UUID (token issued before the stable-sid migration)
            user_session = db.exec(
