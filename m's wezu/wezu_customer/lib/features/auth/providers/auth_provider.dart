...
import 'package:wezu_customer_app/core/services/supabase_auth_service.dart';
...
final authServiceProvider = Provider<AuthService>((ref) => AuthService(
      ref.read(unauthenticatedDioProvider),
...
class AuthNotifier extends StateNotifier<AuthState> {
    ...
    try {
      ...
      if (accessToken != null && accessToken.isNotEmpty) {
        ...
        unawaited(() async {
          ...
          if (!refreshed && (refreshToken != null && refreshToken.isNotEmpty)) {
  ...
  Future<void> _handleAuthSuccess(Map<String, dynamic> result) async {
    ...
    if (accessToken == null) {
      state = state.copyWith(
          isLoading: false, error: 'No token received from server.');
      return;
    }
  ...
  // ── Social login (Supabase OAuth + backend auth/me bootstrap) ─────────────

  /// Sign in with Google.
  ///
  /// Flow:
  ///  1. Supabase handles the native Google OAuth flow and returns a session.
  ///  2. Backend `/auth/me` validates the Supabase access token and resolves
  ///     the mapped customer profile.
  ///  3. App stores Supabase access/refresh tokens for API calls.
  Future<void> loginWithGoogle() async {
    state = state.copyWith(isLoading: true, error: null);
    try {
      final supabaseResponse = await _supabaseAuth.signInWithGoogle();
      final supabaseSession =
          supabaseResponse.session ?? _supabaseAuth.currentSession;
      ...
      final result = await _authService.loginWithSupabaseSession(
      ...
      if (e.isCancelled || e.isWebRedirect) {
  ...
  /// Sign in with Apple.
  ///
  /// Flow mirrors [loginWithGoogle] but uses Apple OAuth via Supabase.
  Future<void> loginWithApple() async {
    state = state.copyWith(isLoading: true, error: null);
    try {
      final supabaseResponse = await _supabaseAuth.signInWithApple();
      final supabaseSession =
          supabaseResponse.session ?? _supabaseAuth.currentSession;
      if (supabaseSession == null) {
        throw const SupabaseAuthException(
          'Apple sign-in completed but no session was returned.',
          code: 'no_session',
        );
      }
      final result = await _authService.loginWithSupabaseSession(
  ...
  Future<void> loginWithBiometrics() async {
    ...
    state = state.copyWith(
        isLoading: true, error: null, failedBiometricAttempts: 0);
    ...
    try {
      ...
      if (fails >= 3) {
        throw Exception(
            'Biometric locked after 3 failures. Use password login.');
      } else {
        rethrow;
      }
    }
  ...
  Future<void> logout() async {
    _inactivityTimer?.cancel();
    // Best-effort: sign out from FastAPI backend and Supabase in parallel.
    await Future.wait([
      _authService.logout().catchError((_) {}),
      _supabaseAuth.signOut().catchError((_) {}),
    ]);
