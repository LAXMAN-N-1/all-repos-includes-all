...
class _StationLocatorScreenState extends ConsumerState<StationLocatorScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    // Single listener handles both cluster update and nearest-station zoom,
    // avoiding two subscriptions on the same provider (which caused redundant
    // rebuild work on every stations fetch).
    ref.listen(nearbyStationsProvider, (previous, next) {
      next.whenData((stations) {
        // 1. Update cluster manager with the latest station list.
        _clusterManager.setItems(stations);
        _clusterManager.updateMap();

        // 2. Auto-zoom to nearest station when the list size changes
