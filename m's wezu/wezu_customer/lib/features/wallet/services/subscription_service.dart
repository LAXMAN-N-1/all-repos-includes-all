...
class SubscriptionService {
  ...
  bool _isEndpointMissing(Object error) =>
      error is DioException && error.response?.statusCode == 404;

  void _logIfUnexpected(String operation, Object error) {
    if (_isEndpointMissing(error)) return;
    debugPrint('SubscriptionService $operation failed: $error');
  }
  ...
  Future<List<SubscriptionPlan>> getSubscriptionPlans() async {
    try {
      ...
      _logIfUnexpected('getSubscriptionPlans', e);
    }
  ...
  Future<Subscription?> getActiveSubscription() async {
    try {
      ...
      _logIfUnexpected('getActiveSubscription', e);
    }
  ...
  Future<SubscriptionPurchaseResponse> purchaseSubscription(
    ...
    try {
      ...
      _logIfUnexpected('purchaseSubscription', e);
    }
  ...
  Future<void> updateAutoRenewal(int subscriptionId, bool autoRenew) async {
    try {
      ...
      _logIfUnexpected('updateAutoRenewal', e);
  ...
  Future<SubscriptionCancellationResponse> cancelSubscription(
    ...
    try {
      ...
      _logIfUnexpected('cancelSubscription', e);
      final now = DateTime.now();
  ...
  Future<SubscriptionPurchaseResponse> manualRenewal(int subscriptionId) async {
    try {
      ...
      _logIfUnexpected('manualRenewal', e);
      if (_localActiveSubscription != null &&
          _localActiveSubscription!.id == subscriptionId) {
  ...
  Future<List<Subscription>> getSubscriptionHistory() async {
    try {
      ...
      _logIfUnexpected('getSubscriptionHistory', e);
      if (_localActiveSubscription != null) {
        return [_localActiveSubscription!];
      }
      return [];
  ...
  Future<bool> validatePlanEligibility(int planId) async {
    try {
      ...
      _logIfUnexpected('validatePlanEligibility', e);
      return false;
    }
  }
}
