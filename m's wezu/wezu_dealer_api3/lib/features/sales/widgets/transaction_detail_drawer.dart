...
class TransactionDetailDrawer extends StatelessWidget {
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  _buildDetailRow('Start Time', TimeUtils.longDateTime(tx.createdAt)),
                  _buildDetailRow('End Time', tx.expectedSettlementDate != null
                      ? TimeUtils.longDateTime(tx.expectedSettlementDate)
                      : 'Ongoing'),
                  ...
                  _buildDetailRow(
                      'Expected Date',
                      tx.expectedSettlementDate != null
                          ? TimeUtils.longDateTime(tx.expectedSettlementDate)
                          : 'TBD'),
                  const SizedBox(height: 32),
  ...
  Widget _buildEnhancedTimeline() {
    ...
    // Use the actual transaction time (local timezone, 24-hour) for the
    // "Rental Started" milestone; all subsequent completed stages share the
    // same creation timestamp since we don't have per-stage times from the API.
    final txTime = TimeUtils.longDateTime(tx.createdAt);

    return Column(
      children: List.generate(stages.length, (index) {
        final isCompleted = index <= currentStage;
        final isLast = index == stages.length - 1;
        // "Rental Started" shows the real time; later completed stages show
        // the same time (best we have); pending stages show '—'.
        String label;
        if (!isCompleted) {
          label = '—';
        } else if (index == 0) {
