...
class _CommissionBreakdown extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final commissions = state.commissions.where((c) {
      final date = DateTime.tryParse(c.createdAt)?.toLocal() ?? now;
...
class _PayoutSchedule extends StatelessWidget {
  ...
  DateTime? _parseDate(String? value) => DateTime.tryParse(value ?? '')?.toLocal();
...
class _RateHistoryDrawer extends StatelessWidget {
  ...
  List<_RateHistoryEntry> _buildEntries() {
    final sorted = [...commissions]..sort((a, b) {
        final ad = DateTime.tryParse(a.createdAt)?.toLocal() ??
            DateTime.fromMillisecondsSinceEpoch(0);
        final bd = DateTime.tryParse(b.createdAt)?.toLocal() ??
            DateTime.fromMillisecondsSinceEpoch(0);
    ...
    for (final c in sorted) {
      final date = DateTime.tryParse(c.createdAt)?.toLocal();
