...
class InventoryBatteriesNotifier extends StateNotifier<InventoryListState> {
  ...
  /// Submit a stock replenishment request to admin
  ...
  /// Fetch stock requests (approved ones pending receipt confirmation)
  Future<List<Map<String, dynamic>>> fetchPendingStockRequests() async {
    try {
      final response = await _dio.get(ApiConstants.stockRequests,
      ...
      if (rawData is Map) {
        ...
        if (inner is List) {
          ...
          rawList = inner['requests'] ?? inner['items'] ?? [];
        }
      ...
      log('Fetch Stock Requests Error: $e');
      return [];
    }
  }

  /// Confirm receipt of stock for a given request ID
  Future<bool> confirmStockReceived(int requestId,
      {int? actualQuantity, String? notes}) async {
    try {
      await _dio.patch(
        '${ApiConstants.stockRequests}/$requestId/receive',
        data: {
          'status': 'received',
      ...
      log('Confirm Stock Received Error: $e');
      return false;
    }
  }
}
