...
class RentalSettingsNotifier extends StateNotifier<RentalSettingsState> {
  ...
  Future<bool> update(Map<String, dynamic> data) async {
    try {
      ...
      // Update backend
      final response =
          await _dio.patch(ApiConstants.rentalSettings, data: data);

      final rawData =
          (response.data is Map && response.data.containsKey('rental_settings'))
...
class SessionsNotifier extends StateNotifier<SessionsState> {
  ...
  Future<bool> revokeSession(int id) async {
    try {
      ...
      await _dio.delete('${ApiConstants.revokeSession}/$id');
      await refresh();
      state = state.copyWith(revokingSessionId: null);
      return true;
    } on DioException catch (e) {
      log('Revoke Session Error: ${e.message}', error: e);
      final detail = ApiResponse.asMap(e.response?.data)['detail']?.toString();
      final fallback = (e.response?.statusCode == 410 ||
              (detail != null && detail.contains('Supabase')))
          ? 'Per-session revoke is managed by Supabase in this backend release.'
          : 'Failed to revoke session';
      state = state.copyWith(
        revokingSessionId: null,
        error: ApiResponse.errorMessage(e, fallback: fallback),
      );
      return false;
    } catch (e) {
