class DealerUser {
  ...
  factory DealerUser.fromJson(Map<String, dynamic> json) {
    ...
    if (json['permissions'] is Map) {
      ...
      for (final raw in (json['permissions'] as List)) {
        ...
        if (parts.length >= 2) {
          ...
          grouped.putIfAbsent('general', () => <String>{}).add(normalized);
        }
      }
      grouped.forEach((key, value) {
        final actions = value.toList()..sort();
        perms[key] = actions;
      });
    }
    ...
    return DealerUser(
      ...
      forcePasswordChange: json['force_password_change'] ??
          ...
          false,
      stationIds: (json['station_ids'] as List?)
