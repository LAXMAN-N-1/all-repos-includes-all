...
// ─── Team Performance ──────────────────────────────────────
// Get team grid (all members with stats)
router.get(
    '/projects/:id/team',
    runValidation(uuidParamValidation),
    ClientController.getTeamPerformance
);

// Get individual employee detail within a project
router.get(
    '/projects/:id/team/:memberId',
    runValidation(uuidParamValidation),
    ClientController.getEmployeeDetail
);

// Get team timeline (weekly hours per member)
router.get(
    '/projects/:id/team-timeline',
    runValidation(uuidParamValidation),
    ClientController.getTeamTimeline
);
