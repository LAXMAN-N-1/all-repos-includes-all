...
router.get(
    '/client-stats',
    requireRole(UserRole.ADMIN, UserRole.PROJECT_MANAGER, UserRole.SCRUM_MASTER, UserRole.CLIENT),
    AnalyticsController.getClientStats
);
...
router.get(
    '/velocity',
    runValidation([query('projectId').optional().isUUID().withMessage('Invalid project ID format')]),
    AnalyticsController.getVelocityChart
);
...
router.get(
    '/db',
    requireRole(UserRole.ADMIN),
    AnalyticsController.getDatabaseStats
);
