...
import { Project, Issue, Milestone, User, Comment, Attachment, Organization, Sprint, ProjectMember, WorkLog } from '../models';
...
export class ClientController {
    ...
    // ─── Team Performance: Grid view ─────────────────────────────────
    // Returns an array of team members with task stats and hours for a project
    static getTeamPerformance = asyncHandler(async (req: AuthRequest, res: Response): Promise<void> => {
        ...
        const teamData = await Promise.all(projectMembers.map(async (pm: any) => {
            const userId = pm.userId;
            const memberUser = pm.user;

            // Count tasks assigned
            const totalTasks = await Issue.count({
                where: { projectId: id, assigneeId: userId, isClientVisible: true },
            });
            const doneTasks = await Issue.count({
                where: { projectId: id, assigneeId: userId, status: 'DONE', isClientVisible: true },
            });
            const inProgressTasks = await Issue.count({
                where: { projectId: id, assigneeId: userId, status: 'IN_PROGRESS', isClientVisible: true },
            });
            const blockedTasks = await Issue.count({
                where: { projectId: id, assigneeId: userId, status: 'BLOCKED', isClientVisible: true },
            });

            // Total hours from WorkLog on project issues
            const hoursResult = await WorkLog.findAll({
                attributes: [
                    [sequelize.fn('COALESCE', sequelize.fn('SUM', sequelize.col('timeSpent')), 0), 'totalHours']
                ],
                where: { userId },
                include: [{
                    model: Issue,
            ...
            else if (totalTasks > 0 && doneTasks === totalTasks) status = 'Completed';
            ...
            return {
                ...
                role: pm.role || memberUser?.role,
                status,
                totalTasks,
                doneTasks,
                inProgressTasks,
                blockedTasks,
                percentDone: totalTasks > 0 ? Math.round((doneTasks / totalTasks) * 100) : 0,
    ...
    // ─── Team Performance: Individual employee detail ─────────────────
    static getEmployeeDetail = asyncHandler(async (req: AuthRequest, res: Response): Promise<void> => {
        ...
        // Current tasks
        const tasks = await Issue.findAll({
            where: { projectId: id, assigneeId: memberId, isClientVisible: true },
            order: [['updatedAt', 'DESC']],
            attributes: ['id', 'key', 'title', 'status', 'priority', 'type', 'storyPoints', 'dueDate', 'updatedAt'],
            include: [
        ...
        workLogs.forEach((wl: any) => {
            const weekStart = new Date(wl.date);
            weekStart.setDate(weekStart.getDate() - weekStart.getDay());
            const key = weekStart.toISOString().split('T')[0] || '';
            weeklyHours[key] = (weeklyHours[key] || 0) + parseFloat(wl.timeSpent);
        });

        // Stats
        const totalTasks = tasks.length;
        const doneTasks = tasks.filter(t => t.status === 'DONE').length;
        const totalHours = workLogs.reduce((sum: number, wl: any) => sum + parseFloat(wl.timeSpent), 0);
        ...
        res.json({
            ...
            data: {
                ...
                tasks,
                weeklyHours: Object.entries(weeklyHours).map(([week, hours]) => ({ week, hours })),
            },
        });
    });

    // ─── Team Performance: Timeline (weekly hours per member) ─────────
    static getTeamTimeline = asyncHandler(async (req: AuthRequest, res: Response): Promise<void> => {
        const { id } = req.params;
        ...
        const timeline = await Promise.all(projectMembers.map(async (pm: any) => {
            const userId = pm.userId;
            ...
            workLogs.forEach((wl: any) => {
                ...
                weeklyData[key] = (weeklyData[key] || 0) + parseFloat(wl.timeSpent);
            });

            return {
                id: userId,
                name: `${pm.user?.firstName} ${pm.user?.lastName}`,
                weeks: Object.entries(weeklyData).map(([week, hours]) => ({
                    week,
