...
export class AuthController {
    ...
    static login = asyncHandler(async (req: AuthRequest, res: Response): Promise<void> => {
        ...
        if (user.mfaEnabled) {
            ...
            // Verify MFA token
            try {
                AuthService.verifyMFAToken(mfaToken, user.id);
            } catch (error) {
    ...
    static changePassword = asyncHandler(async (req: AuthRequest, res: Response): Promise<void> => {
        ...
        user.refreshToken = null; // Invalidate existing refresh sessions
        await user.save();
    ...
    static refreshToken = asyncHandler(async (req: AuthRequest, res: Response): Promise<void> => {
        ...
        // Set cookies
        res.cookie('accessToken', newAccessToken, { httpOnly: true, secure: process.env.NODE_ENV === 'production', sameSite: 'lax', maxAge: 7 * 24 * 60 * 60 * 1000 });
        res.cookie('refreshToken', newRefreshToken, { httpOnly: true, secure: process.env.NODE_ENV === 'production', sameSite: 'lax', maxAge: 30 * 24 * 60 * 60 * 1000 });
    ...
    static resetPassword = asyncHandler(async (req: AuthRequest, res: Response): Promise<void> => {
        ...
        try {
            AuthService.verifyPasswordResetToken(token, user.id);
        } catch (error) {
            throw new AppError('Invalid or expired reset token', 400);
        }
