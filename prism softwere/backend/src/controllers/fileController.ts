...
export class FileController {
    ...
    static uploadProjectFile = asyncHandler(async (req: AuthRequest, res: Response): Promise<void> => {
        ...
        try {
            ...
            if (message.toLowerCase().includes('issueid') && message.toLowerCase().includes('null')) {
                throw new AppError('Project file uploads require the attachment schema migration to be applied', 400);
            }
            throw error;
        }
    ...
    static deleteProjectFile = asyncHandler(async (req: AuthRequest, res: Response): Promise<void> => {
        ...
        const project = attachment.get('project') as Project | undefined;
        if (!project) {
            throw new AppError('Project not found', 404);
        }
        assertProjectAccess(req, project);
