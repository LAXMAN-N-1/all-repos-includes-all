...
class _VendorBidsListScreenState extends ConsumerState<VendorBidsListScreen> {
    ...
    return Scaffold(
      ...
      floatingActionButton: _topVendorIds.isNotEmpty 
          ? FloatingActionButton.extended(
              ...
              label: Text('Confirm Top ${_topVendorIds.length}'),
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
           // Back Button
          TextButton.icon(
            onPressed: () => context.go('/vendor/bidding/dashboard'),
          ...
          Container(
            ...
            decoration: BoxDecoration(
              gradient: const LinearGradient(colors: [AppTheme.emeraldGreen, AppTheme.darkEvergreen]),
            ...
            child: Row(
              ...
              children: [
                ...
                ElevatedButton.icon(
                  onPressed: () => context.push('/vendor/bidding/event-details/${event.id}'),
                  icon: Icon(Icons.visibility, color: AppTheme.emeraldGreen),
                  label: Text('View Event Details', style: TextStyle(color: AppTheme.emeraldGreen)),
                ...
                ElevatedButton.icon(
                  onPressed: () => context.push('/vendor/bidding/comparison/${event.id}'),
                  icon: const Icon(Icons.compare_arrows, color: Colors.white),
                  label: const Text('Compare Bids', style: TextStyle(color: Colors.white)),
                  style: ElevatedButton.styleFrom(backgroundColor: AppTheme.royalAmethyst, padding: const EdgeInsets.symmetric(horizontal: 24, vertical: 16), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
                ),
              ],
            ),
          ),
            ...
            children: [
              _filterCard('Categories', 
                  Wrap(spacing: 8, runSpacing: 8, children: categories.map((cat) => FilterChip(
                      ...
                      side: BorderSide(color: _selectedCategories.contains(cat) ? AppTheme.emeraldGreen : Colors.grey[300]!),
                  )).toList())
              ),
              const SizedBox(width: 16),
              _filterCard('Rating', 
              ...
              rows: filteredBids.map((bid) {
                 ...
                 return DataRow(
                   cells: [
                     ...
                     DataCell(Text(currencyFmt.format(bid.amount), style: const TextStyle(color: AppTheme.emeraldGreen, fontWeight: FontWeight.bold))),
                     ...
                     DataCell(
                       Row(
                         children: [
                           IconButton(icon: Icon(isTop ? Icons.do_not_disturb_on : Icons.add_circle_outline, color: isTop ? AppTheme.error : AppTheme.success), onPressed: () {
                             setState(() {
  ...
  void _showTopVendorsConfirmation(BuildContext context, List<Bid> topBids) {
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: const Text('Confirm Top Vendors'),
      content: SizedBox(
        width: 600,
        child: Column(
      ...
      actions: [
        ...
        ElevatedButton(onPressed: () {
          Navigator.pop(ctx);
          // In real app, call API to set top vendors
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Top vendors lists saved/sent')));
