...
class BidService {
  ...
  Future<List<BiddingEvent>> getDashboardEvents() async {
    final response = await _apiClient.get('/admin/bidding/events');
  ...
  Future<void> pushBidsToCustomer(int eventId, List<int> bidIds) async {
    try {
      await _apiClient.post('/admin/bidding/push-to-customer', data: {
        'event_id': eventId,
        'bid_ids': bidIds,
      });
    } catch (e) {
      throw Exception('Failed to push bids to customer: $e');
    }
  }
}
