...
def seed_demo_users():
    ...
    try:
        ...
        print("🗑️  Deleting all existing Vendor and Customer users...")
        # Get users to delete to handle relationships if needed (though cascade might handle it)
        users_to_delete = db.query(User).filter(User.role_id.in_([vendor_role.id, customer_role.id])).all()
        for user in users_to_delete:
            # Manually delete vendor profile if cascade isn't perfect, 
            # though User model has `cascade="all, delete-orphan"` for vendor_profile
            # explicitly ensuring cleanliness:
        ...
        print("\n" + "="*60)
        print("✅ SUCCESS! Demo Vendor and Customer seeded.")
        print(f"1. Vendor:   mohith@gmail.com / laxman123")
